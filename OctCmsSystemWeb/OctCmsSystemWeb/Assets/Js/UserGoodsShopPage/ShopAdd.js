...
var mExistCompany = false; //判断是否存在公司信息
var mExistAccount = false; //判断是否存在帐号信息

var mUserID = 0; //添加店铺成功后返回的用户UserID
...
function initBuyerMsg() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "1", "UserAccount": UserAccount, "IsAdd": "true",
    };
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                if (reTxt == "52")
                {
                    $("#ShopAccHintDiv").html("<span style=\"color:red\">账号已开通店铺,不能重复添加!</span>");
                    return;
                }
 ...
 * 当父级类别改变时，加载子级菜单列表
 */
function chgLoadSubShopType() {
    var _fatherShopTypeID = $("#ShopTypeIDFather").val().trim();
    //加载店铺父级类别列表
    loadFatherShopType(_fatherShopTypeID, undefined);
}
...
function addShopMsg() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "3", "UserAccount": UserAccount, "CompanyID": CompanyID,
        ...
        "LinkManMobile": LinkManMobile, "LinkEmail": LinkEmail, "IsEntity": IsEntity, "Latitude": LatitudeTxt, "Longitude": LongitudeTxt, "IsSelfShop": IsSelfShop,
    };
    console.log(dataPOST);
    //加载提示
    loadingWin();
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                var _jsonReTxt = JSON.parse(reTxt);
                if (_jsonReTxt.Code == "ASMA_01") {
                    //记录店铺的UserID
                    mUserID = _jsonReTxt.DataDic.UserID
                    console.log("mUserID=" + mUserID);

                    toastWinCb("店铺添加成功,请上传店铺头像和门头照片！", function () {
