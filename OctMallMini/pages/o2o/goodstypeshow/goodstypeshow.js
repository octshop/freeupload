// pages/o2o/goodstypeshow/goodstypeshow.js
//---------------引用文件-----------//
...
Page({
  ...
  onLoad: function (options) {
    ...
    var _goodsTypeIdSec = options.GTIDSe;
    var _goodsTypeNameSec = options.GTNaSe;
    if (_goodsTypeIdSec == undefined) {
      mUtils.switchTabURL("../../../pages/tabbar/shopnear/shopnear");
      return;
    }
    this.setData({
      goodsTypeIdSec: _goodsTypeIdSec,
      goodsTypeNameSec: _goodsTypeNameSec,
    });

    //---------调用自定义函数------//

    //加载第三级商品类目,根据第二级类目ID
    this.loadGoodsTypeThirdBySec();
   ...
   * 加载第三级商品类目,根据第二级类目ID
   */
  loadGoodsTypeThirdBySec: function () {
    ...
    mHttp.postHttp(app.apiURLData.mallApi_GoodsType, _dataPOST, _jsonReTxt => {
      ...
      //设置公共变量
      this.setData({
        loadGoodsTypeThirdBySec_Data: _jsonReTxt,
      });
