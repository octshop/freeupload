...
//AjaxURL
var mAjaxUrl = "../Trading/PayTransRecord";
...
var searchContent = function () {
    ...
    var TransRecordID_se = $("#TransRecordID_se").val().trim();
    var BuyerUserID_se = $("#BuyerUserID_se").val().trim();
    //var OrderID_se = $("#OrderID_se").val().trim();
    var BillNumber_se = $("#BillNumber_se").val().trim();
    var BuyerBankName_se = $("#BuyerBankName_se").val().trim();
    var BuyerBankAcc_se = $("#BuyerBankAcc_se").val().trim();
    var TransMoney_se = $("#TransMoney_se").val().trim();
    var TransMemo_se = $("#TransMemo_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "TransRecordID": TransRecordID_se, "BuyerUserID": BuyerUserID_se, "BillNumber": BillNumber_se,
        "BuyerBankName": BuyerBankName_se, "BuyerBankAcc": BuyerBankAcc_se, "TransMoney": TransMoney_se, "TransMemo": TransMemo_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += " <td>" + indexDataPage.TransRecordID + "<i style=\"color: #939393;display: block\">" + indexDataPage.BuyerUserID + "</i></td>";
        myJsVal += " <td><b>" + indexDataPage.BillNumber + "</b></td>";
        myJsVal += " <td>" + indexDataPage.BankName + "</td>";
        myJsVal += " <td>" + indexDataPage.BuyerBankName + "<i style=\"color: #939393;display: block\">" + indexDataPage.BuyerBankAcc + "</i></td>";
        myJsVal += " <td>" + indexDataPage.TransMoney + "</td>";
        myJsVal += " <td>";
        myJsVal += "     <a href=\"//" + indexDataPage.TransCertImg + "\" target=\"_blank\">";
        myJsVal += "         <img src=\"//" + indexDataPage.TransCertImg + "\" />";
        myJsVal += "     </a>";
        myJsVal += " </td>";
        myJsVal += " <td>" + indexDataPage.TransMemo + "</td>";
        myJsVal += " <td>";
        myJsVal += "" + indexDataPage.WriteDate + "";
        myJsVal += " </td>";
        myJsVal += " <td>";

        if (indexDataPage.IsConfirm == "false") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"confirmTransferPayMul('" + indexDataPage.BillNumber + "', '')\">确认</button>";
        }


        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-danger am-round\" onclick=\"delPayTransRecordArr('" + indexDataPage.TransRecordID + "')\">删除</button>";
        myJsVal += " </td>";
        myJsVal += "</tr>";
 ...
 * 删除信息
 * */
function delPayTransRecordArr(pTransRecordIDArr) {

    confirmWinWidth("确定要删除吗？", function () {

        //构造POST参数
        var dataPOST = {
            "Type": "2", "TransRecordIDArr": pTransRecordIDArr,
        };
 ...
 * 确认收到订单 银行转账金额
 * */
function confirmTransferPayMul(pBillNumber, pShopUserID) {
    ...
    confirmWinWidth("确定收到订单转账金额?", function () {
        ...
        //构造POST参数
        var dataPOST = {
            "Type": "3", "BillNumber": pBillNumber, "ShopUserID": pShopUserID,
        };
        console.log(dataPOST);
        ...
        $.ajax({
            type: "POST",
            url: "../Trading/OrderMsg?rnd=" + Math.random(),
            data: dataPOST,
            dataType: "html",
            success: function (reTxt, status, xhr) {
