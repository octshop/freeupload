...
//AjaxURL
var mAjaxUrl = "../Advertiser/SearchFindMsg";
...
var searchContent = function () {
    ...
    var SearchFindID_se = $("#SearchFindID_se").val().trim();
    var FindType_se = $("#FindType_se").val().trim();
    var IsEntity_se = $("#IsEntity_se").val().trim();
    var SearchContent_se = $("#SearchContent_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "SearchFindID": SearchFindID_se, "FindType": FindType_se, "IsEntity": IsEntity_se,
        "SearchContent": SearchContent_se, "IsLock": IsLock_se, "WriteDate": WriteDate_se
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += " <tr>";
        myJsVal += " <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.SearchFindID + "\" /></td>";
        myJsVal += " <td>" + indexDataPage.SearchFindID + "</td>";
        myJsVal += " <td>" + indexDataPageExtra.FindTypeName + "</td>";
        myJsVal += " <td>";
        myJsVal += "" + indexDataPageExtra.IsEntityName + "";
        myJsVal += " </td>";
        myJsVal += " <td>" + indexDataPage.SearchContent + "</td>";
        myJsVal += " <td>" + indexDataPageExtra.IsLockName + "</td>";
        myJsVal += " <td>" + indexDataPage.WriteDate + "</td>";
        myJsVal += " <td>";
        if (indexDataPage.IsLock == "false") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"tglLock('" + indexDataPage.SearchFindID + "', 'true')\">隐藏</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"tglLock('" + indexDataPage.SearchFindID + "', 'false')\">显示</button>";
        }

        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"goTop('" + indexDataPage.SearchFindID + "')\">置顶</button>";
        myJsVal += " </td>";
        myJsVal += "</tr>";
    }
    //alert(myJsVal);
...
/**
 * 切换 锁定与解锁
 * @param {any} pSearchFindID
 * @param {any} pIsLock 锁定 ( true / false )
 */
function tglLock(pSearchFindID, pIsLock) {

    confirmWinWidth("显示与隐藏操作确认？", function () {

        //构造POST参数
        var dataPOST = {
            "Type": "3", "SearchFindID": pSearchFindID, "IsLock": pIsLock,
        };
 ...
 * @param {any} pIsLock 锁定 ( true / false )
 */
function goTop(pSearchFindID) {

    confirmWinWidth("置顶操作确认？", function () {

        //构造POST参数
        var dataPOST = {
            "Type": "4", "SearchFindID": pSearchFindID,
        };
        console.log(dataPOST);
...
function delSearchFindMsgArr() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "5", "SearchFindIDArr": _selIDArr,
    };
    console.log(dataPOST);
 ...
 * 提交信息
 * */
function submitSearchFindMsg() {

    //获取表单值
    var FindTypeWin = $("#FindTypeWin").val().trim();
    var IsEntityWin = $("#IsEntityWin").val().trim();
    var SearchContentWin = $("#SearchContentWin").val().trim();

    if (SearchContentWin == "") {
        toastWinToDiv("请填写【发现内容】！", "dragWinDiv");
        return;
    }
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "2", "FindType": FindTypeWin, "IsEntity": IsEntityWin, "SearchContent": SearchContentWin, 
    };
    console.log(dataPOST);
