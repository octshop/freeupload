...
$(function () {
    ...
    console.log("mBuyerUserID=" + mBuyerUserID);
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                //如果有拼团折扣，则自动跳转到拼团页面
                if (_jsonReTxt.GroupDiscount != "" && _jsonReTxt.GroupDiscount != undefined && _jsonReTxt.GroupDiscount != null) {
                    if (parseFloat(_jsonReTxt.GroupDiscount) > 0) {
                        window.location.href = "../Group/GroupDetail?GID=" + _jsonReTxt.GoodsMsg.GoodsID;
                        return;
                    }
                }
                ...
                $("#PageTitle").html(_jsonReTxt.GoodsMsg.GoodsTitle + "-商品详情");

                //商品价格
                mGoodsPrice = showDiscountGoodsPrice(_jsonReTxt.GoodsMsgExtra.GoodsPriceLow, _jsonReTxt.GoodsMsg.Discount);
                ...
                if (_jsonReTxt.GoodsMsg.Discount != "0" || _jsonReTxt.GoodsMsg.Discount != "") {
                    $("#Discount").html(_jsonReTxt.GoodsMsg.Discount + "折");
                    //记录当前折扣
                ...
                if (_jsonReTxt.GoodsMsg.IsSpecParam == "true") {
                    ...
                    //设置规格属性窗口中的价格
                    $(".win-spec-top-right #WinSpecPriceDiv").html("&#165;" + showDiscountGoodsPrice(_jsonReTxt.GoodsMsgExtra.GoodsPriceLow, _jsonReTxt.GoodsMsg.Discount));
                }
 ...
 */
var mSelSpecWinHtml = "";
function openSelSpecWin() {

    if (mSelSpecWinHtml == "") {
        mSelSpecWinHtml = getSelSpecWinHtml();
    }
    //初始化SliderDown窗口
    initSilderDownWin(600, mSelSpecWinHtml);
    ...
    for (var i = 0; i < _goodsAbleUseCouponsArr.length; i++) {
        ...
        if (_goodsAbleUseCouponsArr[i].UseMoney != 0 && _goodsAbleUseCouponsArr[i].UseDiscount == 0) {
            if (_goodsAbleUseCouponsArr[i].ExpenseReachSum != 0) {
                ...
                _xhtml += "<b class=\"ticket-b\">" + _goodsAbleUseCouponsArr[i].UseMoney + "元券</b>";
            }
        }
        //折扣券
            ...
            if (_goodsAbleUseCouponsArr[i].ExpenseReachSum != 0) {
                ...
                _xhtml += "<b class=\"ticket-b\">" + _goodsAbleUseCouponsArr[i].UseDiscount + "折券</b>";
            }
        }
    }
...
function xhtmlSelCouponsWin() {
    ...
    for (var i = 0; i < _goodsAbleUseCouponsArr.length; i++) {
        ...
        if (_goodsAbleUseCouponsArr[i].ExpenseReachSum > 0) {
            _expenseReachSum = "消费满" + _goodsAbleUseCouponsArr[i].ExpenseReachSum + "使用";
        }
...
function btnOrderNow(pIsWin) {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "8", "GID": mGID, "SelSpecPropIDVal": SelSpecPropIDVal, "OrderNumber": InputOrderNumber, "BuyerReceiAddrSelCookieArr": _hidBuyerReceiAddrSelCookieArr,
    };
    console.log(dataPOST);
    //加载提示
    $(".btn-order").html("…订购中…");
...
function xhtmlItemGoodsAppraiseImg(pGooAppraiseJsonReTxt) {
    ...
    for (var i = 0; i < pGooAppraiseJsonReTxt.ListGooAppraise.length; i++) {
        ...
        if (GooAppraise.IsAnonymity == "true" || GooAppraise.HeaderImg == "" || GooAppraise.HeaderImg == null) {
...
function countDown() {
    ...
    if (lefttime <= 0) {
        document.querySelector("#SecKillMid").innerHTML = "秒杀已结束";
        return;
    }
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                $("#CusServiceAA").attr("href", "tel:" + ShopMsg.ShopMobile);
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                if (mBuyerGoToImSysURL_GoodsWap != "" && mBuyerGoToImSysURL_GoodsWap != null && mBuyerGoToImSysURL_GoodsWap != undefined) {
                    ...
                    //    window.location.href = encodeURI(mBuyerGoToImSysURL_ShopWap);
                    //});
                    $("#CusServiceAA").attr("href", encodeURI(mBuyerGoToImSysURL_GoodsWap));
