...
//AjaxURL
var mAjaxUrl = "../UserGoodsShop/ActivityJoin";
...
var searchContent = function () {
    ...
    //翻页所用
    var strPOSTSePush = {
        "ActivityID": ActivityID_se, "ShopUserID": ShopUserID_se, "LinkMobile": LinkMobile_se, "UserNick": UserNick_se,
...
function jsonToXhtml(pJsonObject) {
    ...
    var myJsVal = "";    //循环构造    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.AcJoinID + "\" />";        myJsVal += "<input type=\"hidden\" name=\"ActivityIDArr\" value=\"" + indexDataPage.ActivityID + "\" />";        myJsVal += "<input type=\"hidden\" name=\"BuyerUserIDArr\" value=\"" + indexDataPage.BuyerUserID + "\" />";        myJsVal += "</td>";        myJsVal += " <td>" + indexDataPage.ActivityID + "<i style=\"color: gray; display: block;\"><a href=\"../UserGoodsShopPage/ShopMsgDetail?UserID=" + indexDataPage.ShopUserID + "\" target=\"_blank\">" + indexDataPage.ShopUserID + "</a></i></td>";        myJsVal += " <td><a href=\"../UserGoodsShopPage/ActivityDetail?AID=" + indexDataPage.ActivityID + "\" target=\"_blank\">" + indexDataPageExtra.AcTitle + "</a></td>";        myJsVal += " <td><a href=\"tel:" + indexDataPage.LinkMobile + "\">" + indexDataPage.LinkMobile + "</a></td>";        myJsVal += " <td><img src=\"" + indexDataPage.HeaderImg + "\" style=\"width: 30px; height:30px; border-radius:20px; margin-right: 5px;\" />" + indexDataPage.UserNick + "</td>";        myJsVal += " <td>" + _IsChecked + "<i style=\"color: gray; display: block;\">" + _IsLock + "</i></td>";        myJsVal += " <td>" + indexDataPage.CheckedDate + "</td>";        myJsVal += " <td>" + indexDataPage.WriteDate + " <font color=\"red\">" + _WriteDateDay + "</font></td>";        myJsVal += " <td>";        if (indexDataPage.IsLock == "false") {            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"lockActivityJoinArr('" + indexDataPage.AcJoinID + "', 'true')\">锁定</button>";
        }        else {            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"lockActivityJoinArr('" + indexDataPage.AcJoinID + "', 'false')\">解锁</button>";
 ...
 * 批量锁定与解锁 活动参与信息
 */
function lockActivityJoinArr(pAcJoinIDArr, pIsLock) {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "2", "AcJoinIDArr": pAcJoinIDArr, "IsLock": pIsLock,
    };
    ...
    $.ajax({
        ...
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("批量锁定与解锁 活动参与信息=" + reTxt);
            if (reTxt != "") {
