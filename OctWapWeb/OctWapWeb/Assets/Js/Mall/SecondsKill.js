...
//AjaxURL
var mAjaxUrl = "../MallAjax/SecondsKill";
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                for (var i = 0; i < SecKillGoodsTypeList.length; i++) {
                    ...
                    if (_jsonReTxt.ExtraDataList[i].IsHasSkGoodsMsg != "true") {
                        continue;
                    }

                    myJsVal += "<span class=\"goods-type-item\" onclick=\"clickItemSearchWhere1('" + SecKillGoodsTypeList[i].GoodsTypeID + "')\">" + SecKillGoodsTypeList[i].GoodsTypeName + "</span>";
                }
...
function initSearchWhereArr(pPageOrderName, pIsOnlyDiscount) {
    ...
    var mSearchKeyword = "";
    mSearchWhereArr = mSearchKeyword + "^" + pPageOrderName + "^" + pIsOnlyDiscount + "^false^true";
...
function pageSuccess(curPageDataJson) {
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        if (dataPage.GoodsTitle.length > 30) {
            dataPage.GoodsTitle = dataPage.GoodsTitle.substring(0, 30) + "...";
        }
        //---计算价格----//
        var _maxDiscount = 10;
        ...
        //计算百分比
        var _persentProgress = parseInt(parseInt(dataPage.SkSaleNum) / parseInt(dataPage.SkStockNum) * 100);

        //额外信息
        var _extraTxt = "";
        if (dataPageExtra.EndTimeHour != "") {
            _extraTxt += dataPageExtra.EndTimeHour + "结束，";
        }
        _extraTxt += "已抢" + dataPage.SkSaleNum + "件";

        //构造商品标签
        var _goodsLabelSpan = "";
        if (dataPage.EndTime != "" && dataPage.EndTime != null) {
            _goodsLabelSpan += "<span>限时秒杀</span>";
        }
        ...
        myJsVal += "     </div>";
        myJsVal += "     <div class=\"goods-item-progress\">";
        myJsVal += "         <div class=\"progress-bar-bottom\">";
        myJsVal += "             <div class=\"progress-bar-top\" style=\"width:" + _persentProgress + "%\">";
        myJsVal += "                 " + _persentProgress + "%";
        myJsVal += "             </div>";
        myJsVal += "         </div>";
        myJsVal += "         <div class=\"robbed-num\">";
        myJsVal += "" + _extraTxt + "";
        myJsVal += "         </div>";
        myJsVal += "     </div>";
        myJsVal += "     <div class=\"goods-item-badge\">";
        myJsVal += _goodsLabelSpan;
        myJsVal += "     </div>";
        myJsVal += "     <div class=\"goods-item-price\">";
        myJsVal += "         <div>";
        myJsVal += "             <b>&#165; " + formatNumberDotDigit(dataPage.GoodsPrice, 2) + "</b>";
        myJsVal += "             <span>&#165; " + formatNumberDotDigit(_goodsPrice, 2) + "</span>";
        myJsVal += "         </div>";
        myJsVal += "         <input class=\"btn-robbed\" type=\"button\" value=\"马上抢\" />";
        myJsVal += "     </div>";
        myJsVal += " </div>";
        myJsVal += "</li>";
