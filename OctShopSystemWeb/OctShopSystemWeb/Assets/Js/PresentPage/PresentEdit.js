 ...
 * @param {any} pCallBack
 */
function initPresentMsg(pCallBack) {

    var hidPresentID = $("#hidPresentID").val().trim();

    //构造POST参数
    var dataPOST = {
        "Type": "2", "PresentID": hidPresentID,
    };
    ...
    $.ajax({
        type: "POST",
        url: "../Present/PresentEdit?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("初始化礼品信息=" + reTxt);
            if (reTxt != "") {
...
function setValToForm(pJsonInitMsg) {
    ...
    var PresentMsgDetail = pJsonInitMsg.PresentMsgDetail;
    var PresentImgs = pJsonInitMsg.PresentImgs;
    var PresentGoodsTypeList = pJsonInitMsg.PresentGoodsTypeList;

    //赋值商品类目，礼品分类
    $("#hidFirstGoodsTypeID").val(PresentGoodsTypeList.GoodsTypeFirst.GoodsTypeID);
    $("#GoodsTypeSpan_First").html(PresentGoodsTypeList.GoodsTypeFirst.GoodsTypeName);

    $("#hidSecondGoodsTypeID").val(PresentGoodsTypeList.GoodsTypeSecond.GoodsTypeID);
    $("#GoodsTypeSpan_Second").html(PresentGoodsTypeList.GoodsTypeSecond.GoodsTypeName);

    $("#hidThirdGoodsTypeID").val(PresentGoodsTypeList.GoodsTypeThird.GoodsTypeID);
    $("#GoodsTypeSpan_Third").html(PresentGoodsTypeList.GoodsTypeThird.GoodsTypeName);

    //标题
    $("#PresentTitle").val(PresentMsgDetail.PresentTitle);

    //礼品描述
    mEditorContent = PresentMsgDetail.PresentDesc;

    $("#PresentPrice").val(PresentMsgDetail.PresentPrice);
    $("#StockNum").val(PresentMsgDetail.StockNum);

    //初始化商品照片上传 插件
    initGoodsImgUploadList(PresentImgs);

    //到店或快递
    if (PresentMsgDetail.IsShopExpense == "true") {
    ...
    else if (PresentMsgDetail.IsShopExpense == "both") {
        $("#IsShopExpense").prop("checked", true);
        $("#IsExpressHome").prop("checked", true);
    }
    ...
    if (PresentMsgDetail.IsPinkage == "true") {
        $("#IsPinkage").prop("checked", true);
    }
    else {
        $("#IsPinkage").prop("checked", false);
    }

    //兑换须知
    $("#ExchangeNote").val(PresentMsgDetail.ExchangeNote);
...
function submitPresentMsg() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "1", "PresentID": $("#hidPresentID").val().trim(), "GoodsTypeID": hidThirdGoodsTypeID, "PresentTitle": PresentTitle, "PresentPrice": PresentPrice, "StockNum": StockNum, "PresentImgArr": _goodsImgArr, "PresentDesc": PresentDesc, "IsShopExpense": IsShopExpenseVal, "IsPinkage": IsPinkage, "ExchangeNote": ExchangeNote,
    };
    console.log(dataPOST);
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                if (_json.ErrMsg != "" && _json.ErrMsg != null) {
