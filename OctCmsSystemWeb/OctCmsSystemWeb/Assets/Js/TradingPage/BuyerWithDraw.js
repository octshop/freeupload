...
$(function () {
    ...
    if ($("#hidWithDrawStatus").val().trim() != "") {
        var _selOption = $("#WithDrawStatus_se").find("option");
        for (var i = 0; i < _selOption.length; i++) {
            if ($("#hidWithDrawStatus").val().trim() == $(_selOption[i]).val().trim()) {

                $(_selOption[i]).attr("selected", true);
                $("#WithDrawStatus_se").trigger('changed.selected.amui');
                break;
            }
        }
    }
...
var searchContent = function () {
    ...
    var WithDrawID_se = $("#WithDrawID_se").val().trim();
    var SerialNumberD_se = $("#SerialNumberD_se").val().trim();
    var ToType_se = $("#ToType_se").val().trim();
    var WithDrawAmt_se = $("#WithDrawAmt_se").val().trim();
    var WithDrawStatus_se = $("#WithDrawStatus_se").val().trim();
    var TrueName_se = $("#TrueName_se").val().trim();
    var LinkMobile_se = $("#LinkMobile_se").val().trim();
    var WeChatAccount_se = $("#WeChatAccount_se").val().trim();
    var AlipayAccount_se = $("#AlipayAccount_se").val().trim();
    var BankCardNumber_se = $("#BankCardNumber_se").val().trim();
    var BankAccName_se = $("#BankAccName_se").val().trim();
    var OpeningBank_se = $("#OpeningBank_se").val().trim();
    var WithDrawMemo_se = $("#WithDrawMemo_se").val().trim();
    var WxOpenID_se = $("#WxOpenID_se").val().trim();
    var BuyerUserID_se = $("#BuyerUserID_se").val().trim();
    var FinishDate_se = $("#FinishDate_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "WithDrawID": WithDrawID_se, "SerialNumberD": SerialNumberD_se, "ToType": ToType_se, "WithDrawAmt": WithDrawAmt_se, "WithDrawStatus": WithDrawStatus_se, "TrueName": TrueName_se, "LinkMobile": LinkMobile_se, "WeChatAccount": WeChatAccount_se, "AlipayAccount": AlipayAccount_se, "BankCardNumber": BankCardNumber_se, "BankAccName": BankAccName_se,
        "OpeningBank": OpeningBank_se, "WithDrawMemo": WithDrawMemo_se, "WxOpenID": WxOpenID_se, "BuyerUserID": BuyerUserID_se, "FinishDate": FinishDate_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        var _AccVal = indexDataPage.WeChatAccount;
        if (_AccVal == "") {
            _AccVal = indexDataPage.AlipayAccount;
        }
        if (_AccVal == "") {
            _AccVal = indexDataPage.BankCardNumber;
        }

        var _WithDrawStatus = indexDataPage.WithDrawStatus;
        if (indexDataPage.WithDrawStatus == "处理中") {
            _WithDrawStatus = "<font color=\"red\">" + indexDataPage.WithDrawStatus + "</font>";
        }

        myJsVal += "<tr>";
        myJsVal += " <td><a href=\"../TradingPage/BuyerWithDrawDispose?WDID=" + indexDataPage.WithDrawID + "\" target=\"_blank\">" + indexDataPage.WithDrawID + "</a></td>";
        myJsVal += " <td>" + indexDataPage.BuyerUserID + "<i style=\"color: #939393;display: block\">" + indexDataPageExtra.UserNick + "</i></td>";
        myJsVal += " <td>" + indexDataPageExtra.ToTypeName + "</td>";
        myJsVal += " <td>" + indexDataPage.WithDrawAmt + "</td>";
        myJsVal += " <td>" + _WithDrawStatus + "</td>";
        myJsVal += " <td>" + indexDataPage.TrueName + "</td>";
        myJsVal += " <td>" + indexDataPage.LinkMobile + "<i style=\"color: #939393; display: block\">" + indexDataPageExtra.UserAccount + "</i></td>";
        myJsVal += " <td>" + _AccVal + "</td>";
        myJsVal += " <td>";
        myJsVal += "" + indexDataPage.WithDrawMemo + "";
        myJsVal += " </td>";
        myJsVal += " <td>" + indexDataPage.WriteDate + "<i style=\"color: #939393; display: block\">" + indexDataPage.FinishDate + "</i></td>";
        myJsVal += " <td>";

        if (indexDataPage.WithDrawStatus == "完成") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"window.open('../TradingPage/BuyerWithDrawDispose?WDID=" + indexDataPage.WithDrawID + "')\">详细</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-danger am-round\" onclick=\"window.open('../TradingPage/BuyerWithDrawDispose?WDID=" + indexDataPage.WithDrawID + "')\">提现</button>";
        }
