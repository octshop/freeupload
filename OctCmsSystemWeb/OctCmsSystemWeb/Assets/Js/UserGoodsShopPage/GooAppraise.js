...
//AjaxURL
var mAjaxUrl = "../UserGoodsShop/GooAppraise";
...
$(function () {
    ...
    if ($("#hidAppScore").val().trim() != "") {
        var _selOption = $("#AppScore_se").find("option");
        for (var i = 0; i < _selOption.length; i++) {
            if ($("#hidAppScore").val().trim() == $(_selOption[i]).val().trim()) {
...
var searchContent = function () {
    ...
    var UserID = $("#UserID_se").val().trim();
    var ShopUserID = $("#ShopUserID_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "AppScore": AppScore, "OrderID": OrderID, "GoodsID": GoodsID, "AppContent": AppContent, "UserID": UserID, "ShopUserID": ShopUserID,
...
function jsonToXhtml(pJsonObject) {
    ...
    var myJsVal = "";    //循环构造    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        }        myJsVal += " <td>" + indexDataPage.WriteDate + "</td>";        myJsVal += "<td>";        if (indexDataPage.IsLock == "false") {            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"lockGoodsAppraiseArr('" + indexDataPage.AppID + "', 'true')\">锁定</button>";
        }        else {            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"lockGoodsAppraiseArr('" + indexDataPage.AppID + "', 'false')\">解锁</button>";
        }        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"delGooAppraiseArr('" + indexDataPage.OrderID + "', '" + indexDataPage.GoodsID + "')\">删除</button>";        myJsVal += "</td>";        myJsVal += "</tr>";    }    //alert(myJsVal);    //-----分页控制条显示代码-------//    var pageBarXhtml = "";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"PrePage()\">«</a></li>";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('1')\">1</a></li>";    pageBarXhtml += "  <li><span>...</span></li>";    if ((intPageCurrent - 2) > 0) {
 ...
 */
function lockGoodsAppraiseArr(pAppIDArr, pIsLock) {



    //构造POST参数
    var dataPOST = {
        "Type": "2", "AppIDArr": pAppIDArr, "IsLock": pIsLock,
    };
 ...
 * 删除评价信息
 */
function delGooAppraiseArr(pOrderID, pGoodsID) {

    if (pOrderID == "" || pGoodsID == "" || pOrderID == "0" || pGoodsID == "0") {
        return
    }
    ...
    confirmWinWidth("确定要删除吗？", function () {
        ...
        //构造POST参数
        var dataPOST = {
            "Type": "3", "OrderID": pOrderID, "GoodsID": pGoodsID,
        };
        console.log(dataPOST);
