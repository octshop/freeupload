...
//AjaxURL
var mAjaxUrl = "../UserGoodsShop/LuckyDrawMsg";
...
var searchContent = function () {
    ...
    var strPOSTSePush = {
        ...
        "LuckydrawStatus": LuckydrawStatus_se, "StartLuckyType": StartLuckyType_se, "ExpressType": ExpressType_se, "JoinLimit": JoinLimit_se, "IsPauseJoin": IsPauseJoin_se, "StartLuckyTime": StartLuckyTime_se, "WriteDate": WriteDate_se, "ShopUserID": ShopUserID_se, "IsCheck": IsCheck_se, "IsLock": IsLock_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    var myJsVal = "";    //循环构造    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.LuckydrawID + "\" />";        myJsVal += " <td>" + indexDataPage.LuckydrawID + "<i style=\"color: gray; display: block;\"><a href=\"../UserGoodsShopPage/ShopMsgDetail?UserID=" + indexDataPage.ShopUserID + "\" target=\"_blank\">" + indexDataPage.ShopUserID + "</a></i></td>";        myJsVal += " <td><a href=\"" + mhidOctWapWeb_AddrDomain + "/Mall/PagePreMobileIframe?LoadPreURL=" + mhidOctWapWeb_AddrDomain + "/Mall/LuckyDrawDetail?LID=" + indexDataPage.LuckydrawID + "\" target=\"_blank\">" + indexDataPage.LuckydrawTitle + "</a></td>";        myJsVal += " <td>" + indexDataPage.LuckydrawStatus + "<i style=\"color: gray; display: block;\">" + _IsLock + "</i></td>";        myJsVal += " <td>" + _StartLuckyType + "</td>";        myJsVal += " <td>" + _JoinLimit + "</td>";        myJsVal += " <td>" + _IsPauseJoin + "<br /><font color=\"red\">" + indexDataPageExtra.JoinTotalNumber + "</font></td>";        myJsVal += " <td>" + _IsCheck + "<i style=\"color: gray; display: block;\">" + indexDataPage.CheckReason + "</i></td>";        myJsVal += " <td>" + indexDataPage.StartLuckyTime + " <font color=\"red\">" + _StartLuckyTimeDay + "</font></td>";        myJsVal += " <td>" + indexDataPage.WriteDate + "</td>";        myJsVal += " <td>";        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"window.open('../UserGoodsShopPage/LuckyDrawMsgDetail?LID=" + indexDataPage.LuckydrawID + "')\">详情</button>";        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"openEditWin('" + indexDataPage.LuckydrawID + "', '" + indexDataPage.LuckydrawTitle + "', '" + indexDataPage.IsCheck + "', '" + indexDataPage.CheckReason + "')\">审核</button>";        myJsVal += " </td>";        myJsVal += "</tr>";    }    //alert(myJsVal);    //-----分页控制条显示代码-------//    var pageBarXhtml = "";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"PrePage()\">«</a></li>";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('1')\">1</a></li>";    pageBarXhtml += "  <li><span>...</span></li>";    if ((intPageCurrent - 2) > 0) {
 ...
 * 批量锁定与解锁 抽奖信息
 */
function lockLuckydrawMsgArr(pLuckydrawIDArr, pIsLock) {

    if (pLuckydrawIDArr == "") {
        pLuckydrawIDArr = getSelectValArr();
    }
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "3", "LuckydrawIDArr": pLuckydrawIDArr, "IsLock": pIsLock,
    };
    ...
    $.ajax({
        ...
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("批量锁定与解锁 抽奖信息=" + reTxt);
            if (reTxt != "") {
 ...
 * 打开编辑信息窗口
 */
function openEditWin(pLuckydrawIDWin, pLuckydrawTitleWin, pIsCheckWin, pCheckReasonWin) {

    //打开Dialog弹出窗口
    openDialogWinNoClose("审核窗口", mAddEditWinHtml, function () {

        //提交审核
        chkLuckyDrawMsg(pLuckydrawIDWin);
    ...
    //初始化窗口
    $("#LuckydrawIDWin").html("" + pLuckydrawIDWin + "(" + pLuckydrawTitleWin + ")");
 ...
 * 提交审核
 * */
function chkLuckyDrawMsg(pLuckydrawID) {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "2", "LuckydrawID": pLuckydrawID, "IsCheck": IsCheckWin, "CheckReason": CheckReasonWin,
    };
    console.log(dataPOST);
