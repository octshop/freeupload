...
//AjaxURL
var mAjaxUrl = "../CommonCode/KeyVerifyRecord";
...
var searchContent = function () {
    ...
    var VRecordID_se = $("#VRecordID_se").val().trim();
    var UserKeyID_se = $("#UserKeyID_se").val().trim();
    var UserKey_se = $("#UserKey_se").val().trim();
    var RndKey_se = $("#RndKey_se").val().trim();
    var VerifyType_se = $("#VerifyType_se").val().trim();
    var VerifyTypeArr_se = $("#HttpServerIP_se").val().trim();
    var LangType_se = $("#LangType_se").val().trim();
    var IsVerify_se = $("#IsVerify_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "VRecordID": VRecordID_se, "UserKeyID": UserKeyID_se,
        "RndKey": RndKey_se, "UserKey": UserKey_se, "VerifyType": VerifyType_se,
        "LangType": LangType_se, "IsVerify": IsVerify_se,
...
function jsonToXhtml(pJsonObject) {
    ...
    var myJsVal = "";    //循环构造    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.VRecordID + "\" /></td>";        myJsVal += "  <td>" + indexDataPage.VRecordID +"</td>";        myJsVal += "  <td><a href=\"javascript:openDtailWin(" + indexDataPage.UserKeyID +")\">" + indexDataPage.UserKeyID + "</a></td>";        myJsVal += "  <td>" + indexDataPage.UserKey +"</td>";        myJsVal += "  <td>" + indexDataPage.RndKey +"</td>";        myJsVal += "  <td>" + indexDataPage.VerifyType +"</td>";        myJsVal += "  <td>" + indexDataPage.HttpServerIP +"</td>";        myJsVal += "  <td>" + indexDataPage.LangType +"</td>";        myJsVal += "  <td>" + indexDataPage.IsVerify +"</td>";        myJsVal += "  <td>" + indexDataPage.IsLock +"</td>";        myJsVal += "  <td>" + indexDataPage.WriteDate +"</td>";        myJsVal += "  <td>";        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning  am-round\" onclick=\"toggleLock(" + indexDataPage.VRecordID +")\">";        if (indexDataPage.IsLock == "否") {            myJsVal += "<span class=\"am-icon-unlock\"></span>"
...
/**
 * 开关锁定
 * @param {any} pVRecordID 调用记录ID
 */
function toggleLock(pVRecordID) {

    //构造POST参数
    var dataPOST = {
        "Type": "2", "VRecordID": pVRecordID,
    };
