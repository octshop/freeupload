...
/**
 * 切换分页类型
 * @param pTypePage 分页类型Type值 4, 5
 * */
function chgTab(pTypePage) {

    mTypePage = pTypePage;

    $("#chgTab_1").removeClass("oct-header-sub-curren-a");
    $("#chgTab_2").removeClass("oct-header-sub-curren-a");
    $(event.currentTarget).addClass("oct-header-sub-curren-a");
...
function pageSuccess(curPageDataJson) {
    ...
    var dataPageArr = curPageDataJson.DataPage1;
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        if (mTypePage == "4") {
            _orderID = dataPage.OrderID;
            _orderTitle = "订单ID：";
            _finishPayDate = "完成时间：<b>" + formatDataTimeShortDate(dataPage.FinishTime) + "</b>";
        }
        else if (mTypePage == "5") {
            _orderID = dataPage.AggregateOrderID;
            _orderTitle = "扫码订单ID：";
            _finishPayDate = "支付时间：<b>" + formatDataTimeShortDate(dataPage.PayTime) + "</b>";
        }

        myJsVal += "<li class=\"settle-msg-li\" onclick=\"window.location.href='../WapPage/OrderDetail?OID=" + dataPage.OrderID + "'\">";
        myJsVal += " <div class=\"settle-top\">";
        myJsVal += "     <div>" + _orderTitle + "<b>" + _orderID + "</b> 金额：<b>&#165;" + dataPage.OrderPrice + "</b></div>";
        myJsVal += "     <div>";
        myJsVal += "         <b>" + dataPage.OrderStatus + "</b>";
        myJsVal += "     </div>";
        myJsVal += " </div>";
        myJsVal += " <div class=\"settle-content\">";
        myJsVal += "     <div>";
        myJsVal += "         <span>交易号：</span><b>" + dataPage.BillNumber + "</b>";
        myJsVal += "     </div>";
        myJsVal += "     <div>";
        myJsVal += "         <span>抽成金额：</span><b>&#165;" + dataPage2.CommissionMoney + "</b>";
        myJsVal += "         <span class=\"right-span\">结算金额：</span><b>&#165;" + (parseFloat(dataPage.OrderPrice) - parseFloat(dataPage2.CommissionMoney)) + "</b>";
        myJsVal += "     </div>";
        myJsVal += " </div>";
        myJsVal += " <div class=\"settle-date\">";
        myJsVal += "     <div>";
        myJsVal += "         支付方式：<b>" + dataPageExtra.PayWayName + "</b>";
        myJsVal += "     </div>";
        myJsVal += "     <div>";
        myJsVal += "" + _finishPayDate + "";
        myJsVal += "     </div>";
        myJsVal += " </div>";
        myJsVal += "</li>";
    }
