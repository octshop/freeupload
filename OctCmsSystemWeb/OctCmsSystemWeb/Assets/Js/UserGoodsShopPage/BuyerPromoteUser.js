...
/**-----定义公共变量------**/
//AjaxURL
var mAjaxUrl = "../UserGoodsShop/BuyerPromoteUser";
...
var searchContent = function () {
    ...
    var PromoteID_se = $("#PromoteID_se").val().trim();
    var BuyerUserID_se = $("#BuyerUserID_se").val().trim();
    var BuyerUserIDPromoted_se = $("#BuyerUserIDPromoted_se").val().trim();
    var UserNick_se = $("#UserNick_se").val().trim();
    var BindMobile_se = $("#BindMobile_se").val().trim();
    var UserNickPromoted_se = $("#UserNickPromoted_se").val().trim();
    var BindMobilePromoted_se = $("#BindMobilePromoted_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "PromoteID": PromoteID_se, "BuyerUserID": BuyerUserID_se, "BuyerUserIDPromoted": BuyerUserIDPromoted_se,
        "UserNick": UserNick_se, "BindMobile": BindMobile_se, "UserNickPromoted": UserNickPromoted_se, "BindMobilePromoted": BindMobilePromoted_se, "IsLock": IsLock_se, "WriteDate": WriteDate_se
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += " <tr>";
        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.PromoteID + "\" /></td>";
        myJsVal += " <td>" + indexDataPage.PromoteID + "</td>";
        myJsVal += " <td>" + indexDataPage.BuyerUserID + "<i style=\"color:gray;display:block;\">" + indexDataPage.BuyerUserIDPromoted + "</i></td>";
        myJsVal += " <td>" + indexDataPage.BindMobile + "<i style=\"color:gray;display:block;\">" + indexDataPage.BindMobilePromoted + "</i></td>";
        myJsVal += " <td><div class=\"td-header-img\"><img src=\"" + indexDataPage.HeaderImg + "\" />" + indexDataPage.UserNick + "</div></td>";
        myJsVal += " <td><div class=\"td-header-img\"><img src=\"" + indexDataPage.HeaderImgPromoted + "\" />" + indexDataPage.UserNickPromoted + "</div></td>";
        myJsVal += " <td>" + indexDataPageExtra.IsLockName + "</td>";
        myJsVal += " <td>" + indexDataPage.WriteDate + "</td>";
        myJsVal += " <td>";

        if (indexDataPage.IsLock == "false") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"lockBuyerPromoteUser('" + indexDataPage.PromoteID + "', 'true')\">锁定</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"lockBuyerPromoteUser('" + indexDataPage.PromoteID + "', 'false')\">解锁</button>";
        }
 ...
 * @param {any} pIsLock
 */
function lockBuyerPromoteUser(pPromoteID, pIsLock) {

    //构造POST参数
    var dataPOST = {
        "Type": "2", "PromoteID": pPromoteID, "IsLock": pIsLock,
    };
 ...
 * 批量删除信息
 */
function delBuyerPromoteUserArr() {
 ...
 * 提交信息
 * */
function submitBuyerPromoteUser() {

    //获取表单值
    var BuyerUserIDWin = $("#BuyerUserIDWin").val().trim();
    var BuyerUserIDPromotedWin = $("#BuyerUserIDPromotedWin").val().trim();

    if (BuyerUserIDWin == "" || BuyerUserIDPromotedWin == "") {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "4", "BuyerUserID": BuyerUserIDWin, "BuyerUserIDPromoted": BuyerUserIDPromotedWin,
    };
    console.log(dataPOST);
