...
//AjaxURL
var mAjaxUrl = "../Advertiser/AdvBanner";
...
var searchContent = function () {
    ...
    var BannerID_se = $("#BannerID_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "BannerID": BannerID_se, "AdvType": AdvType_se, "AdvTitleType": AdvTitleType_se,
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.UploadGuid + "\" />";
        myJsVal += " <td>" + indexDataPage.BannerID + "</td>";
        myJsVal += " <td>" + indexDataPageExtra.AdvTypeName + "</td>";
        myJsVal += " <td>";
        myJsVal += "" + indexDataPageExtra.AdvTitleTypeName + "";
        myJsVal += " </td>";
        ...
        if (indexDataPage.IsLock == "false") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"tglLock('" + indexDataPage.BannerID + "', 'true')\">隐藏</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"tglLock('" + indexDataPage.BannerID + "', 'false')\">显示</button>";
        }


        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"goTop('" + indexDataPage.BannerID + "')\">置顶</button>";
        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-danger am-round\" onclick=\"openEditWin('" + indexDataPage.BannerID + "')\">修改</button>";
        myJsVal += " </td>";
        myJsVal += "</tr>";
 ...
 */
function tglLock(pBannerID, pIsLock) {

    confirmWinWidth("显示与隐藏操作确认？", function () {

        //构造POST参数
        var dataPOST = {
            "Type": "4", "BannerID": pBannerID, "IsLock": pIsLock,
        };
 ...
 * @param {any} pIsLock 锁定 ( true / false )
 */
function goTop(pBannerID) {

    confirmWinWidth("置顶操作确认？", function () {

        //构造POST参数
        var dataPOST = {
            "Type": "5", "BannerID": pBannerID,
        };
        console.log(dataPOST);
 ...
 */
function openEditWin(pBannerID) {
...
function submitAdvBanner() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "2", "UploadGuid": mImgKeyGuid, "ImgURL": mImgPathDomain, "BannerID": mBannerID, "AdvType": AdvTypeWin, "AdvTitleType": AdvTitleTypeWin, "AdvOsType": AdvOsTypeWin, "OverTime": OverTimeWin, "AdvLinkType": AdvLinkTypeWin, "AdvLinkA": AdvLinkAWin, "AdvMemo": AdvMemoWin, "SelCityRegionCodeArr": SelCityRegionCodeArr,
    };
    console.log(dataPOST);
 ...
 * 初始化广告图片信息
 * */
function initAdvBannerDetail() {
    //构造POST参数
    var dataPOST = {
        "Type": "3", "BannerID": mBannerID,
    };
    console.log(dataPOST);
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                $("#BannerPreImg").show();
                var myJsVal = "";
                myJsVal += " <a href=\"//" + mImgPathDomain + "\" target=\"_blank\"><img id=\"PreImgBanner\" style=\"width: 380px; height: 150px;\" src=\"//" + mImgPathDomain + "\" /></a>";
...
function initUploadImg() {
    ...
    $('#fileupload_1').fileupload({
        url: "../FileUpload/AdvBannerImg?" + _dataPost + "&rnd=" + Math.random(),
        //dataType: 'json',
        dataType: "text",
        //------------处理上传成功后的事件------------//
            ...
            $("#BannerCropBtn").show();
            $("#BannerCropBtn").attr("href", "//" + _domain1 + "/ToolWeb/CropZoom/CropZoom.aspx?CropImgWidth=514&CropImgHeight=200&CropTitle=图片裁剪&ImgSourceURL=" + _imgPath1 + "&RedirectURL=#");
...
function delUploadImg() {
    ...
    $.ajax({
        type: "POST",
        url: "../FileUpload/AdvBannerImg?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
