...
//AjaxURL
var mAjaxUrl = "../AfterSaleAccCus/AfterSaleApplyMsg";
...
$(function () {
    ...
    if ($("#hidApplyStatus").val().trim() != "") {
        var _selOption = $("#ApplyStatus_se").find("option");
        for (var i = 0; i < _selOption.length; i++) {
            if ($("#hidApplyStatus").val().trim() == $(_selOption[i]).val().trim()) {
...
var searchContent = function () {
    ...
    var SpecID_se = $("#SpecID_se").val().trim();
    ...
    var ProblemDesc_se = $("#ProblemDesc_se").val().trim();
    var ApplyMemo_se = $("#ApplyMemo_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "ApplyID": ApplyID_se, "OrderID": OrderID_se, "GoodsID": GoodsID_se, "SpecID": SpecID_se, "ApplyStatus": ApplyStatus_se, "ServiceType": ServiceType_se, "ServiceWay": ServiceWay_se, "ApplyReason": ApplyReason_se, "ProblemDesc": ProblemDesc_se, "ApplyMemo": ApplyMemo_se, "BuyerUserID": BuyerUserID_se, "ShopUserID": ShopUserID_se, "IsLock": IsLock_se, "FinishTime": FinishTime_se, "OrderTime": OrderTime_se, "WriteDate": WriteDate_se,
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += " <td>" + indexDataPage.ApplyID + "<i style=\"color: #939393;display:block\"><a href=\"../TradingPage/OrderDetail?OID=" + indexDataPage.OrderID + "\" target=\"_blank\">" + indexDataPage.OrderID + "</a></i></td>";
        myJsVal += " <td><a href=\"" + mOctWapWebAddrDomain + "/Goods/GoodsDetailPreMobileIframe?GID=" + indexDataPage.GoodsID + "\" target=\"_blank\">" + indexDataPage.GoodsID + "</a><i style=\"color: #939393;display: block\">" + indexDataPage.SpecID + "</i></td>";
        myJsVal += " <td><a href=\"../UserGoodsShopPage/UserAccountDetail?UID=" + indexDataPage.BuyerUserID + "\" target=\"_blank\">" + indexDataPage.BuyerUserID + "</a><i style=\"color: #939393;display:block\"><a href=\"../UserGoodsShopPage/ShopMsgDetail?UserID=" + indexDataPage.ShopUserID + "\" target=\"_blank\">" + indexDataPage.ShopUserID + "</a></i></td>";
        myJsVal += " <td>" + indexDataPage.ApplyStatus + "<i style=\"color: #939393;display:block\">" + indexDataPageExtra.ServiceTypeName + "</i></td>";
        myJsVal += " <td>" + indexDataPageExtra.ServiceWayName + "<i style=\"color: #939393;display:block\">" + indexDataPage.ApplyReason + "</i></td>";
        myJsVal += " <td>" + indexDataPage.ApplyGoodsNum + "<i style=\"color: #939393;display:block\">" + indexDataPageExtra.IsLockName + "</i></td>";
        myJsVal += " <td>" + indexDataPage.FinishTime + "<i style=\"color: #939393;display:block\">" + indexDataPage.OrderTime + "</i></td>";
        myJsVal += " <td>";
        ...
        if (indexDataPage.IsLock == "false") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"locklAfterSaleApplyMsg('" + indexDataPage.ApplyID + "')\">锁定</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"locklAfterSaleApplyMsg('" + indexDataPage.ApplyID + "')\">解锁</button>";
        }
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"window.location.href='../AfterSaleAccCusPage/AfterSaleDetail?AID=" + indexDataPage.ApplyID + "'\">详情</button>";
        myJsVal += " </td>";
        myJsVal += "</tr>";
 ...
 * @param {any} pApplyID
 */
function locklAfterSaleApplyMsg(pApplyID) {
