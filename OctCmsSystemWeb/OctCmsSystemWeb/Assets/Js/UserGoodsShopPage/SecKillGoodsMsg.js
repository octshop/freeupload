...
/**-----定义公共变量------**/
//AjaxURL
var mAjaxUrl = "../UserGoodsShop/SecKillGoodsMsg";
...
var searchContent = function () {
    ...
    //翻页所用
    var strPOSTSePush = {
        "ShopUserID": ShopUserID_se, "GoodsID": GoodsID_se, "SkDiscount": SkDiscount_se, "SkStockNum": SkStockNum_se,
        "SkSaleNum": SkSaleNum_se, "IsCheck": IsCheck_se, "IsUnSale": IsUnSale_se, "IsLock": IsLock_se, "EndTime": EndTime_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += " <td>" + indexDataPage.SkSaleNum + "<i style=\"color: gray; display: block;\"><a href=\"../UserGoodsShopPage/ShopMsgDetail?UserID=" + indexDataPage.ShopUserID +"\" target=\"_blank\">" + indexDataPage.ShopUserID + "</a></i></td>";
        myJsVal += " <td>";
        myJsVal += _checkStatu;
        myJsVal += "<div style=\"color:gray;font-size:10px;\">" + indexDataPage.CheckReason + "</div>";
        myJsVal += " </td>";
        myJsVal += " <td>" + _isUnSale + "<i style=\"color: gray; display: block;\">" + _IsLock + "</i></td>";
        ...
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"openEditWin('" + indexDataPage.GoodsID + "', '" + indexDataPageExtra.GoodsTitle + "','" + indexDataPage.IsCheck + "', '" + indexDataPage.CheckReason + "')\">审核</button>";
 ...
 * 批量锁定与解锁 秒杀商品信息
 */
function lockSecKillGoodsMsgArr(pSkGoodsIDArr, pIsLock) {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "3", "SkGoodsIDArr": pSkGoodsIDArr, "IsLock": pIsLock,
    };
    ...
    $.ajax({
        ...
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("批量锁定与解锁 秒杀商品信息=" + reTxt);
            if (reTxt != "") {
...
function openEditWin(pGoodsIDWin, pGoodsTitleWin, pIsCheckWin, pCheckReasonWin) {
    ...
    openDialogWinNoClose("审核窗口", mAddEditWinHtml, function () {
        ...
        //提交审核
        chkSecKillGoodsMsg(pGoodsIDWin);
 ...
 * 提交审核
 * */
function chkSecKillGoodsMsg(pGoodsID) {
