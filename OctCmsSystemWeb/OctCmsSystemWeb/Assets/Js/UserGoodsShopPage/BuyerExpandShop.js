...
/**-----定义公共变量------**/
//AjaxURL
var mAjaxUrl = "../UserGoodsShop/BuyerExpandShop";
...
var searchContent = function () {
    ...
    //翻页所用
    var strPOSTSePush = {
        "BuyerUserID": BuyerUserID_se, "UserNick": UserNick_se, "LinkMobile": LinkMobile_se,
        "ShopID": ShopID_se, "ShopUserID": ShopUserID_se, "ShopName": ShopName_se, "ShopMobile": ShopMobile_se, "IsLock": IsLock_se, "WriteDate": WriteDate_se
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.ExpandShopID + "\" /></td>";
        myJsVal += " <td>" + indexDataPage.BuyerUserID + "</td>";
        myJsVal += " <td>" + indexDataPage.UserNick + "<i style=\"color:gray;display:block;\">" + indexDataPage.LinkMobile + "</i></td>";
        myJsVal += " <td>" + indexDataPage.ShopID + "<i style=\"color:gray;display:block;\">" + indexDataPage.ShopUserID + "</i></td>";
        myJsVal += " <td><div onclick=\"window.open('" + mOctWapWebAddrDomain + "/Shop/Index?SID=" + indexDataPage.ShopID + "')\" class=\"td-header-img\" style=\"cursor:pointer\"><img src=\"//" + indexDataPage.ShopHeaderImg + "\" />" + indexDataPage.ShopName + "</div></td>";
        myJsVal += " <td>" + indexDataPage.ShopMobile + "</td>";
        myJsVal += " <td>" + _IsLock + "</td>";
        myJsVal += " <td>" + indexDataPage.WriteDate + "</td>";

        myJsVal += " <td>";
        if (indexDataPage.IsLock == "false") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"lockBuyerExpandShop('" + indexDataPage.ExpandShopID + "', 'true')\">锁定</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"lockBuyerExpandShop('" + indexDataPage.ExpandShopID + "', 'false')\">解锁</button>";
        }
        myJsVal += " </td>";
 ...
 * @param {any} pIsLock
 */
function lockBuyerExpandShop(pExpandShopIDArr, pIsLock) {

    //构造POST参数
    var dataPOST = {
        "Type": "2", "ExpandShopIDArr": pExpandShopIDArr, "IsLock": pIsLock,
    };
 ...
 * 批量删除信息
 */
function delBuyerExpandShopArr() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "3", "ExpandShopIDArr": MsgIDArr,
    };
    console.log(dataPOST);
...
function submitBuyerExpandShop() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "4", "BuyerUserID": BuyerUserIDWin, "ShopID": ShopIDWin,
    };
    console.log(dataPOST);
