...
$(function () {
    ...
    if ($("#hidSysMsgType").val().trim() != "") {
        var _selOrderStatusOption = $("#SysMsgType_se").find("option");
        for (var i = 0; i < _selOrderStatusOption.length; i++) {
            if ($("#hidSysMsgType").val().trim() == $(_selOrderStatusOption[i]).val().trim()) {

                $(_selOrderStatusOption[i]).attr("selected", true);
                $("#SysMsgType_se").trigger('changed.selected.amui');

            }
        }
    }
    if ($("#hidIsRead").val().trim() != "") {
        var _selOrderStatusOption = $("#IsRead_se").find("option");
        for (var i = 0; i < _selOrderStatusOption.length; i++) {
            if ($("#hidIsRead").val().trim() == $(_selOrderStatusOption[i]).val().trim()) {

                $(_selOrderStatusOption[i]).attr("selected", true);
                $("#IsRead_se").trigger('changed.selected.amui');
...
var searchContent = function () {
    ...
    //翻页所用
    var strPOSTSePush = {
        "SSysMsgID": SSysMsgID_se, "SysMsgType": SysMsgType_se, "MsgTitle": MsgTitle_se,
        "MsgContent": MsgContent_se, "OrderID": OrderID_se, "ExtraData": ExtraData_se, "IsRead": IsRead_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += " <td>" + indexDataPageExtra.SysMsgTypeName + "(Order)</td>";
        ...
        myJsVal += " <td><a href=\"../" + indexDataPageExtra.ExtraLinkA + "\" target=\"_blank\">" + indexDataPage.ExtraData + "</a></td>";

        myJsVal += " <td>" + indexDataPageExtra.IsReadName + "</td>";
