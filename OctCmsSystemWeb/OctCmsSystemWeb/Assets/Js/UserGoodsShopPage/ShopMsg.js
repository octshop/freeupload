//================店铺信息========================//

/**-----定义公共变量------**/

//AjaxURL
var mAjaxUrl = "../UserGoodsShop/ShopMsg";

/******数据分页的变量********/
var strPOSTSe = ""; //搜索条件对象 POST参数
var intPageCurrent = 1; //当前页
var pageSize = 0; //当页的记录条数，与后台保持一致
var recordSum = 0; //总记录数
var tableColNum = 11; //当前表列数

//信息ID
var mMsgID = "0";

//添加或编辑窗口的Html显示代码
var mAddEditWinHtml = "";


/**------初始化------**/
$(function () {

    if ($("#hidIsCheck").val().trim() != "") {
        var _selOption = $("#IsCheck_se").find("option");
        for (var i = 0; i < _selOption.length; i++) {
            if ($("#hidIsCheck").val().trim() == $(_selOption[i]).val().trim()) {

                $(_selOption[i]).attr("selected", true);
                $("#IsCheck_se").trigger('changed.selected.amui');
                break;
            }
        }
    }

    //初始化地区选择
    initRegionCodeName();

    //初始化加载
    searchContent();

    //搜索按钮单击事件
    $("#btnSearch").click(function () {
        searchContent();
    });

    //初始化添加窗口显示代码
    initAddEditWinHtml();

});


/*
---------定义搜索函数---------
*/
var searchContent = function () {

    intPageCurrent = 1; //开始页

    var ShopID_se = $("#ShopID_se").val().trim();
    var UserID_se = $("#UserID_se").val().trim();
    var CompanyID_se = $("#CompanyID_se").val().trim();
    var ShopTypeID_se = $("#ShopTypeID_se").val().trim();
    var ShopName_se = $("#ShopName_se").val().trim();
    var LinkMan_se = $("#LinkMan_se").val().trim();
    var LinkManMobile_se = $("#LinkManMobile_se").val().trim();
    var ShopMobile_se = $("#ShopMobile_se").val().trim();
    var MajorGoods_se = $("#MajorGoods_se").val().trim();

    //选择地区
    var region_province = $("#region_province").val();
    var region_city = $("#region_city").val();
    var region_county = $("#region_county").val();
    var RegionCodeArr = "";
    if (region_county == null || region_county == "") {
        RegionCodeArr = "";
    }
    else {
        RegionCodeArr = region_province + "_" + region_city + "_" + region_county;
    }


    var IsCheck_se = $("#IsCheck_se").val().trim();
    var IsShow_se = $("#IsShow_se").val().trim();
    var IsLock_se = $("#IsLock_se").val().trim();
    var WriteDate_se = $("#WriteDate_se").val().trim();


    //构造POST参数
    var strPOST = {
        "pageCurrent": "1", "Type": "1"
    };

    //翻页所用
    var strPOSTSePush = {
        "ShopID": ShopID_se, "UserID": UserID_se, "CompanyID": CompanyID_se,
        "ShopTypeID": ShopTypeID_se, "ShopName": ShopName_se, "LinkMan": LinkMan_se,
        "LinkManMobile": LinkManMobile_se, "ShopMobile": ShopMobile_se, "MajorGoods": MajorGoods_se,
        "RegionCodeArr": RegionCodeArr, "IsCheck": IsCheck_se,
        "IsShow": IsShow_se, "IsLock": IsLock_se, "WriteDate": WriteDate_se
    };
    //将对象添加到分类对象中

    //搜索内容用
    var strPOSTSeContent = pushTwoObject(strPOST, strPOSTSePush);

    //分页操作用
    var strPOSTSearch = { "Type": "1" };
    strPOSTSe = pushTwoObject(strPOSTSearch, strPOSTSePush);
    console.log(strPOSTSe);

    //加载提示
    $("#TbodyTrPage").html("<tr><td colspan=\"" + tableColNum + "\">… 数据加载中 …</td></tr>");

    //以POST方式发送异步请求
    $.ajax({
        type: "POST",
        url: mAjaxUrl + "?rnd=" + Math.random(),
        data: strPOSTSeContent,
        dataType: "html",
        success: function (reTxtJson, status, xhr) {
            //显示返回值
            console.log(reTxtJson);
            if (reTxtJson != "") {

                //分页成功返回，构造显示代码
                pageSuccess(reTxtJson);

            }
            else {
                //加载提示
                $("#TbodyTrPage").html("<tr><td colspan=\"" + tableColNum + "\">没有搜索到相关数据</td></tr>");
            }
        },
        error: function (xhr, errorTxt, status) {
            console.log("错误信息errorTxt=" + errorTxt + " | status=" + status);
            //alert("网络出现异常,请重试！");
        }

    });
};


//------------解析JSON数据 构造 前台显示代码--------------//
// pJsonTxt Json字符串
function jsonToXhtml(pJsonObject) {

    //将字符串转换成功JSON对象
    //var json = JSON.parse(pJsonTxt);
    var json = pJsonObject;

    //-----内容显示前台显示代码----//
    var myJsVal = "";
    //循环构造
    for (var i = 0; i < json.DataPage.length; i++) {

        var indexDataPage = json.DataPage[i];
        var indexDataPageExtra = json.DataPageExtra[i];

        var _IsCheck = indexDataPage.IsCheck;
        if (indexDataPage.IsCheck == "true") {
            _IsCheck = "<font color=\"red\">审核中</font>"
        }
        else {
            _IsCheck = "通过";
        }

        myJsVal += "<tr>";
        myJsVal += "  <td>" + indexDataPage.ShopID + "<br /><a href=\"../UserGoodsShopPage/UserAccountDetail?UID=" + indexDataPage.UserID + "\" target=\"_blank\">" + indexDataPage.UserID + "</a></td>";
        myJsVal += "  <td>" + indexDataPage.CompanyID + "<i> - <a href=\"../UserGoodsShopPage/CompanyMsgDetail?CompanyID=" + indexDataPage.CompanyID + "\" target=\"_blank\">" + indexDataPageExtra.CompanyName + "</a></i></td>";
        myJsVal += "  <td>" + indexDataPage.ShopTypeID + " <i> - " + indexDataPageExtra.ShopTypeName + " </i></td>";
        myJsVal += "  <td><a href=\"../UserGoodsShopPage/ShopMsgDetail?UserID=" + indexDataPage.UserID + "\" target=\"_blank\">" + indexDataPage.ShopName + "</a></td>";
        myJsVal += "  <td>" + indexDataPage.RegionNameArr + "</td>";
        myJsVal += "  <td>" + indexDataPage.LinkMan + "</td>";
        myJsVal += "  <td>" + indexDataPage.LinkManMobile + "<i> - 联</i><br />";
        myJsVal += "" + indexDataPage.ShopMobile + "<i> - 店</i></td>";
        myJsVal += "  <td>" + indexDataPage.MajorGoods + "</td>";
        myJsVal += "  <td>" + _IsCheck + " / " + indexDataPage.IsShow + " / " + indexDataPage.IsLock + "</td>";
        myJsVal += "  <td>" + indexDataPage.WriteDate + "</td>";
        myJsVal += "  <td>";

        //审核
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"openEditWin(" + indexDataPage.ShopID + ")\"><span class=\"";
        if (indexDataPage.IsCheck == "true") {
            myJsVal += "am-icon-close";
        }
        else {
            myJsVal += "am-icon-check";
        }
        myJsVal += "\"></span></button>";

        //显示/隐藏
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"showShopMsg(" + indexDataPage.ShopID + ")\"><span class=\"";
        if (indexDataPage.IsShow == "true") {
            myJsVal += "am-icon-star"
        }
        else {
            myJsVal += "am-icon-star-o"
        }
        myJsVal += "\"></span></button>"

        //编辑
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"window.location.href='../UserGoodsShopPage/ShopEdit?UserID=" + indexDataPage.UserID + "'\"><span class=\"am-icon-pencil-square-o";
        myJsVal += "\"></span></button>"

        //锁定
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning  am-round\" onclick=\"lockShopMsg(" + indexDataPage.ShopID + ")\"><span id=\"BtnToggleLock\" class=\"";
        if (indexDataPage.IsLock == "true") {
            myJsVal += "am-icon-lock"
        }
        else {
            myJsVal += "am-icon-unlock"
        }
        myJsVal += "\"></span></button>"

        myJsVal += "  </td>";
        myJsVal += "</tr>";

    }
    //alert(myJsVal);

    //-----分页控制条显示代码-------//
    var pageBarXhtml = "";
    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"PrePage()\">«</a></li>";
    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('1')\">1</a></li>";
    pageBarXhtml += "  <li><span>...</span></li>";


    if ((intPageCurrent - 2) > 0) {
        pageBarXhtml += "  <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('" + (intPageCurrent - 2) + "')\">" + (intPageCurrent - 2) + "</a></li>";
    }
    if ((intPageCurrent - 1) > 0) {
        pageBarXhtml += "  <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('" + (intPageCurrent - 1) + "')\">" + (intPageCurrent - 1) + "</a></li>";
    }


    pageBarXhtml += "  <li class=\"am-active\"><a href=\"javascript:void(0)\" onclick=\"NumberPage('" + json.PageCurrent + "')\">" + json.PageCurrent + "</a></li>";

    console.log(parseInt(json.PageSum));

    if ((intPageCurrent + 1) <= parseInt(json.PageSum)) {
        pageBarXhtml += "  <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('" + (intPageCurrent + 1) + "')\">" + (intPageCurrent + 1) + "</a></li>";
    }
    if ((intPageCurrent + 2) <= parseInt(json.PageSum)) {
        pageBarXhtml += "  <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('" + (intPageCurrent + 2) + "')\">" + (intPageCurrent + 2) + "</a></li>";
    }


    pageBarXhtml += "  <li><span>...</span></li>";
    pageBarXhtml += "  <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('" + json.PageSum + "')\">" + json.PageSum + "</a></li>";
    pageBarXhtml += "  <li><input type=\"text\" id=\"PageNumTxt\" class=\"page-go-text am-form-field\" placeholder=\"跳转页\" /></li>";
    pageBarXhtml += "  <li><a href=\"javascript:void(0)\" onclick=\"NextPage()\">»</a></li>";




    var _pageMsgArr = new Array()
    //内容显示代码 
    _pageMsgArr[0] = myJsVal;
    //控制条件显示代码
    _pageMsgArr[1] = pageBarXhtml;
    //返回数组
    return _pageMsgArr;
}


//------------------------------搜索结果分页-------------------------------//
//具体页
function NumberPage(pagecurrent) {
    intPageCurrent = parseInt(pagecurrent);

    //以GET方式发送分页请求的函数
    sendPageHttpGet(intPageCurrent);
}

//上一页
function PrePage() {
    intPageCurrent = intPageCurrent - 1;

    if (intPageCurrent > 0) {

        //以GET方式发送分页请求的函数
        sendPageHttpGet(intPageCurrent);

    }
    else {
        intPageCurrent = 1;
    }
}


//下一页
function NextPage() {
    intPageCurrent = parseInt(intPageCurrent) + 1;

    //计算总页数
    var intPageSum = recordSum % pageSize != 0 ? recordSum / pageSize + 1 : recordSum / pageSize;
    if (intPageCurrent <= parseInt(intPageSum)) {

        //以GET方式发送分页请求的函数
        sendPageHttpGet(intPageCurrent);

    }
    else {
        intPageCurrent = parseInt(intPageSum);
    }

}

//----------------以GET方式发送分页请求的函数-----------------//
var sendPageHttpGet = function (intPageCurrent) {
    //构造GET参数
    strPOSTSe = pushTwoObject(strPOSTSe, { "pageCurrent": intPageCurrent });

    //加载提示
    $("#TbodyTrPage").html("<tr><td colspan=\"" + tableColNum + "\">… 数据加载中 …</td></tr>");

    $.ajax({
        type: "GET",
        url: mAjaxUrl + "?rnd=" + Math.random(),
        data: strPOSTSe,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            //成功返回后的处理函数
            pageSuccess(reTxt)
        },
        error: function (xhr, errorTxt, status) {
            console.log("异步请求错误，errorTxt=" + errorTxt + " | status=" + status);
            //alert("网络出现异常,请重试！");
            return;
        }
    });
}


//------------分页信息成功返回------------//
// @ reTxtJson 返回的Json字符串
function pageSuccess(reTxtJson) {
    if (reTxtJson != "") {

        var reJsonObject = JSON.parse(reTxtJson);
        console.log(reJsonObject);

        //总的记录数
        recordSum = reJsonObject.RecordSum;
        pageSize = reJsonObject.PageSize;


        //解析JSON数据 构造 前台显示代码
        var _xhtmlArr = jsonToXhtml(reJsonObject);

        //显示内容
        $("#TbodyTrPage").html(_xhtmlArr[0]);
        //分页控制条
        $("#PageBar1").html(_xhtmlArr[1]);

        //滚动条回到顶部
        document.body.scrollTop = 0;


        //跳转页 获取当文本框获取了焦点，按了键盘事件
        $("#PageNumTxt").keydown(function (event) {
            //alert(event.keyCode);
            if (event.keyCode == "13") {

                //跳转方法
                gotoPage();

                return false;
            }
        });

        //全不选
        //document.getElementById("SelAllCb").checked = false;

    }
    else {
        $("#TbodyTrPage").html("<tr><td colspan=\"" + tableColNum + "\">没有搜索到相关数据</td></tr>");
    }
}


//---------------------------跳转页---------------------//
function gotoPage() {

    var pageNum = $("#PageNumTxt").val()
    //判断输入的页数是否小于0
    if (parseInt(pageNum) <= 0) {
        toastWin("跳转的页数不能小于或等于0!");
        return;
    }

    //判断页输入框是否为空
    if (pageNum == "" || pageNum == null) {
        toastWin("请输入要跳转的页数！");
        return;
    }
    //判断输入的页是否为数字
    if (isNaN(pageNum)) {
        toastWin("跳转的页数必须是数字！");
        return;
    }

    //调用数据库函数
    NumberPage(pageNum);
}



//---------------------弹出窗口功能-------------//

/**
 * 初始化添加窗口显示代码
 */
function initAddEditWinHtml() {
    //获取窗口显示代码
    mAddEditWinHtml = $("#AddEditWin").html();
    $("#AddEditWin").empty();
}


/**
 * 打开编辑窗口
 * @param {any} mMsgID 信息ID
 */
function openEditWin(pMsgID) {

    mMsgID = pMsgID;

    //打开Dialog弹出窗口
    openDialogWinNoClose("审核店铺信息", mAddEditWinHtml, function () {

        //确定按钮
        aeMsg("edit")

    }, function () {


    }, 600);


    //初始化信息
    initShopMsg(pMsgID);
}

/**
 * 初始化信息
 * @param {any} pMsgID 信息ID
 */
function initShopMsg(pMsgID) {

    //构造POST参数
    var dataPOST = {
        "Type": "4", "ShopID": pMsgID,
    };
    console.log(dataPOST);

    //正式发送异步请求
    $.ajax({
        type: "POST",
        url: mAjaxUrl + "?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log(reTxt);
            if (reTxt != "") {
                var _jsonReTxt = JSON.parse(reTxt).ShopMsg;
                mMsgID = pMsgID;

                $("#ShopID_ae").html("" + _jsonReTxt.ShopID + " ，" + _jsonReTxt.ShopName + "");
                $("#IsCheck_ae").val(_jsonReTxt.IsCheck);
                $("#CheckReason_ae").val(_jsonReTxt.CheckReason);
            }

        },
        error: function (xhr, errorTxt, status) {
            console.log("异步请求错误，errorTxt=" + errorTxt + " | status=" + status);
            //alert("异步请求错误，errorTxt=" + errorTxt + " | status=" + status);
            return;
        }
    });
}


/**
 * 添加或编辑信息 (审核操作)
* @param {any} pExeType 操作类型 [add / edit]
 */
function aeMsg(pExeType) {

    //获取表单值
    var IsCheck_ae = $("#IsCheck_ae").val().trim();
    var CheckReason_ae = $("#CheckReason_ae").val().trim();

    //构造POST参数
    var dataPOST = {
        "Type": "5", "ShopID": mMsgID, "IsCheck": IsCheck_ae, "CheckReason": CheckReason_ae,
    };
    console.log(dataPOST);

    //显示加载提示
    loadingWinToDiv("dragWinDiv");

    //以POST方式发送异步请求
    $.ajax({
        type: "POST",
        url: mAjaxUrl + "?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxtJson, status, xhr) {
            //显示返回值
            console.log(reTxtJson);
            //移除加载提示
            closeLoadingWin();

            if (reTxtJson != "") {

                var _jsonObj = JSON.parse(reTxtJson);
                if (_jsonObj.Code != null) {

                    //提示信息
                    toastWinToDiv(_jsonObj.Msg, "dragWinDiv");
                    //关闭弹出窗口
                    closeDialogWin();
                    //重置信息ID
                    var mMsgID = "0";
                    //重新加载数据
                    NumberPage(intPageCurrent);

                    return
                }

                //显示错误信息
                if (_jsonObj.ErrCode != null) {
                    toastWinToDiv(_jsonObj.ErrMsg, "dragWinDiv");
                }
            }
        },
        error: function (xhr, errorTxt, status) {
            console.log("错误信息errorTxt=" + errorTxt + " | status=" + status);
            //alert("网络出现异常,请重试！");
        }
    });
}

/**
 * 显示/隐藏 店铺信息
 * @param {any} pMsgID
 */
function showShopMsg(pMsgID) {


    //构造POST参数
    var dataPOST = {
        "Type": "6", "ShopID": pMsgID,
    };
    console.log(dataPOST);

    //以POST方式发送异步请求
    $.ajax({
        type: "POST",
        url: mAjaxUrl + "?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxtJson, status, xhr) {
            //显示返回值
            console.log(reTxtJson);

            if (reTxtJson != "") {

                var _jsonObj = JSON.parse(reTxtJson);
                if (_jsonObj.Code != null) {

                    //提示信息
                    toastWin(_jsonObj.Msg);
                    //重新加载数据
                    NumberPage(intPageCurrent);

                    return
                }

                //显示错误信息
                if (_jsonObj.ErrCode != null) {
                    toastWin(_jsonObj.ErrMsg);
                }
            }
        },
        error: function (xhr, errorTxt, status) {
            console.log("错误信息errorTxt=" + errorTxt + " | status=" + status);
            //alert("网络出现异常,请重试！");
        }
    });
}

/**
 * 锁定/解锁 店铺信息
 * @param {any} pMsgID
 */
function lockShopMsg(pMsgID) {


    //构造POST参数
    var dataPOST = {
        "Type": "7", "ShopID": pMsgID,
    };
    console.log(dataPOST);

    //以POST方式发送异步请求
    $.ajax({
        type: "POST",
        url: mAjaxUrl + "?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxtJson, status, xhr) {
            //显示返回值
            console.log(reTxtJson);

            if (reTxtJson != "") {

                var _jsonObj = JSON.parse(reTxtJson);
                if (_jsonObj.Code != null) {

                    //提示信息
                    toastWin(_jsonObj.Msg);
                    //重新加载数据
                    NumberPage(intPageCurrent);

                    return
                }

                //显示错误信息
                if (_jsonObj.ErrCode != null) {
                    toastWin(_jsonObj.ErrMsg);
                }
            }
        },
        error: function (xhr, errorTxt, status) {
            console.log("错误信息errorTxt=" + errorTxt + " | status=" + status);
            //alert("网络出现异常,请重试！");
        }
    });
}

