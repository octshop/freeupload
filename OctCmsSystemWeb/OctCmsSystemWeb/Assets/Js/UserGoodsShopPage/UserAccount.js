...
var searchContent = function () {
    ...
    var UserID_se = $("#UserID_se").val().trim();
    var UserAccount_se = $("#UserAccount_se").val().trim();
    var BindMobile_se = $("#BindMobile_se").val().trim();
    var UserNick_se = $("#UserNick_se").val().trim();
    var VipLevel_se = $("#VipLevel_se").val().trim();
    var WxOpenID_se = $("#WxOpenID_se").val().trim();
    var MiniOpenID_se = $("#MiniOpenID_se").val().trim();
    var WxUnionID_se = $("#WxUnionID_se").val().trim();
    var AccountMemo_se = $("#AccountMemo_se").val().trim();
    var IsOpenShop_se = $("#IsOpenShop_se").val().trim();
    var ShopName_se = $("#ShopName_se").val().trim();
    var IsTransDividend_se = $("#IsTransDividend_se").val().trim();
    var IsLock_se = $("#IsLock_se").val().trim();
    var WriteDate_se = $("#WriteDate_se").val().trim();

    var CurrentRegionCodeArr = "";
    try {
        var region_province = $("#region_province").val().trim();
        var region_city = $("#region_city").val().trim();
        var region_county = $("#region_county").val().trim();
        if (region_county != "") {
            CurrentRegionCodeArr = region_province + "_" + region_city + "_" + region_county;
        }
    } catch (e) {
        CurrentRegionCodeArr = "";
    }
    ...
    //翻页所用
    var strPOSTSePush = {
        "UserID": UserID_se, "UserAccount": UserAccount_se, "BindMobile": BindMobile_se,
        "UserNick": UserNick_se, "VipLevel": VipLevel_se, "WxOpenID": WxOpenID_se, "MiniOpenID": MiniOpenID_se, "WxUnionID": WxUnionID_se, "AccountMemo": AccountMemo_se, "IsOpenShop": IsOpenShop_se, "ShopName": ShopName_se, "IsTransDividend": IsTransDividend_se, "CurrentRegionCodeArr": CurrentRegionCodeArr, "IsLock": IsLock_se, "RegDate": WriteDate_se
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage1.length; i++) {
        ...
        var indexDataPage3 = json.DataPage3[i];
        var indexDataPageExtra = json.DataPageExtra[i];

        myJsVal += "<tr>";
        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage1.UserID + "\" /></td>";
        myJsVal += " <td>" + indexDataPage1.UserID + "<i style=\"color:gray;display:block;\">" + indexDataPage1.UserNick + "</i></td>";
        myJsVal += " <td>" + indexDataPage1.UserAccount + "<i style=\"color:gray;display:block;\">" + indexDataPage1.BindMobile + "</i></td>";
        myJsVal += " <td>" + indexDataPageExtra.VipLevelName + "<i style=\"color:gray;display:block;\">" + indexDataPage1.CreditScore + "</i></td>";
        myJsVal += " <td>" + indexDataPage1.WxOpenID + "<i style=\"color:gray;display:block;\">" + indexDataPage1.MiniOpenID + "</i></td>";
        myJsVal += " <td>";
        myJsVal += "" + indexDataPageExtra.IsOpenShopName + "<i style=\"color: gray;display: block;\"><a href=\"" + mOctWapWebAddrDomain + "/Mall/PagePreMobileIframe?LoadPreURL=" + mOctWapWebAddrDomain + "/Shop/Index?SID=" + indexDataPage3.ShopID + "\" target=\"_blank\">" + indexDataPage3.ShopName + "</a></i>";
        myJsVal += " </td>";
        myJsVal += " <td>" + indexDataPage1.AccountMemo + "</td>";
        myJsVal += " <td>" + indexDataPageExtra.IsLockName + "<i style=\"color:gray;display:block;\">" + indexDataPageExtra.IsTransDividendName + "</i></td>";
        myJsVal += " <td>" + indexDataPage1.RegDate + "</td>";
        myJsVal += " <td>";
        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"window.open('../UserGoodsShopPage/UserAccountDetail?UID=" + indexDataPage1.UserID + "')\">详情</button>";
        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning am-round\" onclick=\"openEditWin('" + indexDataPage1.UserID + "','" + indexDataPage1.UserNick + "','" + indexDataPage1.IsTransDividend + "','" + indexDataPage1.AccountMemo + "')\">修改</button>";
        myJsVal += " </td>";
        myJsVal += "</tr>";
    }
    //alert(myJsVal);
 ...
 */
function tglLockUserAccountArr(pIsLock) {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "2", "UserIDArr": MsgIDArr, "IsLock": pIsLock,
    };
    ...
    $.ajax({
        ...
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("批量-锁定/解锁会员账号=" + reTxt);
            if (reTxt != "") {
 ...
 */
function openEditWin(pMsgID, pUserNick, pIsTransDividend, pAccountMemo) {
    ...
    //窗口显示赋值
    $("#UserIDWin").html("" + pMsgID + " (" + pUserNick + ")");
    $("#IsTransDividendWin").val(pIsTransDividend);
    $("#AccountMemoWin").val(pAccountMemo);
}
...
function editUserAccount() {
    ...
    //获取表单值
    var IsTransDividendWin = $("#IsTransDividendWin").val().trim();
    var AccountMemoWin = $("#AccountMemoWin").val().trim();

    //构造POST参数
    var dataPOST = {
        "Type": "3", "UserID": mMsgID, "IsTransDividend": IsTransDividendWin, "AccountMemo": AccountMemoWin,
    };
    console.log(dataPOST);
