...
//AjaxURL
var mAjaxUrl = "../AfterSaleAccCus/BuyerSysMsg";
...
var searchContent = function () {
    ...
    var BSysMsgID_se = $("#BSysMsgID_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "BSysMsgID": BSysMsgID_se, "BuyerUserID": BuyerUserID_se, "SysMsgType": SysMsgType_se,
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += " <td>" + indexDataPage.BSysMsgID + "</td>";
        myJsVal += " <td>" + indexDataPage.BuyerUserID + "<br /><a href=\"../UserGoodsShopPage/UserAccountDetail?UID=" + indexDataPage.BuyerUserID + "\" target=\"_blank\">" + indexDataPageExtra.UserNick + "</a></td>";
 ...
 */
function delBuyerSysMsgMonthAgo(pMonthNumAgo) {
    ...
    confirmWinWidth("确定要删除吗？", function () {
        ...
        $.ajax({
            ...
            dataType: "html",
            success: function (reTxt, status, xhr) {
                console.log("删除指定几个月之前的通知信息=" + reTxt);
                if (reTxt != "") {
