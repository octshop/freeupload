...
function saveCompanyMsg() {
    ...
    if (CreditCode == "" || LegalPerson == "" || RegMoney == "" || SetUpDate == "" || BusiScope == "") {
        toastWin("所有项都必须填写！");
        return;
    }
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "6", "CompanyID": hidCompanyID, "CreditCode": CreditCode, "LegalPerson": LegalPerson,
...
function saveCompanyCertificateImg(pCompanyID, pCertType, pCertImg, pUploadGuid) {
    ...
    $.ajax({
        type: "POST",
        url: "../shop/CompanyCertificate?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log(reTxt);
