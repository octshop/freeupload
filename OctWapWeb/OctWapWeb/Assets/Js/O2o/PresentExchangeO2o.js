...
//AjaxURL
var mAjaxUrl = "../O2oAjax/PresentExchange";
...
//排序方式
var mOrderBy = "Distance";

//当前价格排序
var mPresentPriceSort = "PresentPriceAsc";

var mSearchKeyword = ""; //搜索关键字

var mGoodsTypeIDThird = ""; //第三级礼品类目ID
...
function loadPresentGoodsType() {
    //构造POST参数
    var dataPOST = {
        "Type": "2", "IsEntity": "true",
    };
    console.log(dataPOST);
    //正式发送异步请求
    $.ajax({
        type: "POST",
        url: "../MallAjax/PresentExchange?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("加载礼品显示分类=" + reTxt);
            if (reTxt != "") {
                ...
                for (var i = 0; i < PresentGoodsTypeList.length; i++) {
                    myJsVal += "<span class=\"goods-type-item\" onclick=\"clickItemSearchWhere1('" + PresentGoodsTypeList[i].GoodsTypeID + "')\">" + PresentGoodsTypeList[i].GoodsTypeName + "</span>";
                }
 ...
 * 统计买家当前积分总额
 * */
function loadCurrentIntegralBuyer() {
    //构造POST参数
    var dataPOST = {
        "Type": "2",
    };
    ...
    $.ajax({
        ...
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("统计买家当前积分总额=" + reTxt);
            if (reTxt != "") {
                var _jsonReTxt = JSON.parse(reTxt);

                $("#UserNickB").html(_jsonReTxt.UserNick);
                $("#CurIntegralB").html(_jsonReTxt.CurrentIntegral);
            }
        }
    });
}
...
function chgTabPrice() {
    ...
    if (mPresentPriceSort == "PresentPriceAsc") {
        mPresentPriceSort = "PresentPriceDesc";
        //切换选项卡
        clickItemSearchWhere3('PresentPriceDesc')

    }
    else {
        mPresentPriceSort = "PresentPriceAsc";
        //切换选项卡
        clickItemSearchWhere3('PresentPriceAsc')
    }
...
function initSearchWhereArr() {
    ...
    //mSearchWhereArr = mRegionCountyCode + "^" + mOrderBy + "^" + mSearchKeyword;
...
function pageSuccess(curPageDataJson) {
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        if (dataPage.PresentTitle.length > 34) {
            dataPage.PresentTitle = dataPage.PresentTitle.substring(0, 34) + "…";
        }

        var _bageLabelSpan = "";
        if (dataPage.IsShopExpense == "true") {
            _bageLabelSpan += "<span>到店消费/自取</span>";
        }
        else if (dataPage.IsShopExpense == "both") {
            _bageLabelSpan += "<span>到店或快递物流</span>";
        }
        else if (dataPage.IsShopExpense == "false") {
            _bageLabelSpan += "<span>快递物流</span>";
        }
        if (dataPage.IsShopExpense != "true" && dataPage.IsPinkage == "true") {
            _bageLabelSpan += "<span>包邮</span>";
        }
        if (dataPage.IsEntity == "true") {
            _bageLabelSpan += "<span>实体店</span>";
        }

        myJsVal += " <li class=\"goods-item\" onclick=\"window.location.href='../Present/PresentDetail?PID=" + dataPage.PresentID + "'\">";
        myJsVal += " <div class=\"goods-img\">";
        myJsVal += "     <img src=\"//" + dataPage.ImgURLCover + "\" />";
        myJsVal += " </div>";
        myJsVal += " <div>";
        myJsVal += "     <div class=\"present-title-div\">";
        myJsVal += "" + dataPage.PresentTitle + "";
        myJsVal += "     </div>";
        myJsVal += "     <div class=\"goods-price\">";
        myJsVal += "         积分：" + dataPage.PresentPrice + "";
        myJsVal += "     </div>";
        myJsVal += "     <div class=\"goods-label\">";
        myJsVal += _bageLabelSpan;
        myJsVal += "     </div>";
        myJsVal += "     <div class=\"goods-extra\">";
        myJsVal += "         " + dataPage.ShopName + "<a href=\"javascript:void(0)\" onclick=\"goShopIndex('" + dataPage.ShopID + "')\">进店>></a>";
        myJsVal += "     </div>";
        myJsVal += " </div>";
        myJsVal += "</li>";
