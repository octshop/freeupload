...
/**-----定义公共变量------**/
var mAjaxUrl = "../AfterSale/ShopSysMsg";
...
function pageSuccess(curPageDataJson) {
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        var _cssCurrent = "";
        if (dataPage.IsRead == "false") {
            _cssCurrent = "order-dynamic-item-current";
        }

        var _onclickstr = "";
        if (dataPage.SysMsgType == "Order" || dataPage.SysMsgType == "ORemind") {
            _onclickstr = "onclick=\"window.location.href='../WapPage/OrderDetail?OID=" + dataPage.ExtraData + "'\"";
        }
        else if (dataPage.SysMsgType == "AfterSale" || dataPage.SysMsgType == "AsRemind") {
            _onclickstr = "onclick=\"window.location.href='../AfterSalePage/AfterSaleDetail?AID=" + dataPage.ExtraData + "'\"";
        }

        myJsVal += "<div class=\"order-dynamic-item " + _cssCurrent + "\" " + _onclickstr + ">";
        myJsVal += " <div class=\"dynamic-item-left\">";
        myJsVal += formatDateTimeSysMsg(dataPage.WriteDate);
        myJsVal += " </div>";
        myJsVal += " <div class=\"dynamic-item-mid\">";
        myJsVal += "     <div class=\"dynamic-circle\"></div>";
        myJsVal += " </div>";
        myJsVal += " <div class=\"dynamic-item-right\">";
        myJsVal += "     <div class=\"msg-title\">";
        myJsVal += "         " + dataPage.MsgTitle + "";
        myJsVal += "     </div>";
        myJsVal += "     " + dataPage.MsgContent + "";
        myJsVal += " </div>";
        myJsVal += "</div>";
...
/**
 * 格式化的时间日期
 * @param {any} pChatDateTime
 */
function formatDateTimeSysMsg(pChatDateTime) {

    var dateTimeStr = new Date(pChatDateTime);
    var yearStr = dateTimeStr.getFullYear(); //年
    var monthStr = dateTimeStr.getMonth() + 1; //月
    var dateStr = dateTimeStr.getDate(); //日

    var hourStr = dateTimeStr.getHours();
    var minutesStr = dateTimeStr.getMinutes();
    var secondStr = dateTimeStr.getSeconds();

    var _formatDateTime = yearStr + "-" + monthStr + "-" + dateStr + "<br />" + hourStr + ":" + minutesStr + ":" + secondStr;
