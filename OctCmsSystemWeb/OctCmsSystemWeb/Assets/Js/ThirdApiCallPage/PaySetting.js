...
//AjaxURL
var mAjaxUrl = "../ThirdApiCall/PaySetting";
...
var searchContent = function () {
    ...
    var PaySettingID_se = $("#PaySettingID_se").val().trim();
    var UserKeyID_se = $("#UserKeyID_se").val().trim();
    var PayType_se = $("#PayType_se").val().trim();
    var NotifyUrl_se = $("#NotifyUrl_se").val().trim();
    var ReturnUrl_se = $("#ReturnUrl_se").val().trim();
    var QuitUrl_se = $("#QuitUrl_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "PaySettingID": PaySettingID_se, "UserKeyID": UserKeyID_se, "PayType": PayType_se,
        "NotifyUrl": NotifyUrl_se, "ReturnUrl": ReturnUrl_se, "QuitUrl": QuitUrl_se, "WriteDate": WriteDate_se
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += " <td>" + indexDataPage.PaySettingID + "</td>";
        myJsVal += " <td>" + indexDataPage.UserKeyID + "</td>";
        myJsVal += " <td>";
        myJsVal += "" + indexDataPageExtra.PayTypeName + "(" + indexDataPage.PayType + ")";
        myJsVal += " </td>";
        myJsVal += " <td>" + indexDataPage.NotifyUrl + "</td>";
        myJsVal += " <td>" + indexDataPage.ReturnUrl + "</td>";
        myJsVal += " <td>" + indexDataPage.QuitUrl + "</td>";
        myJsVal += " <td>" + indexDataPage.WriteDate + "</td>";
        myJsVal += " <td>";
        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"openEditWin('" + indexDataPage.PaySettingID + "', '" + indexDataPage.UserKeyID + "', '" + indexDataPageExtra.PayTypeName + "(" + indexDataPage.PayType + ")', '" + indexDataPage.NotifyUrl + "', '" + indexDataPage.ReturnUrl + "', '" + indexDataPage.QuitUrl + "')\">修改</button>";
        myJsVal += " </td>";
        myJsVal += "</tr>";
 ...
 * 打开编辑窗口
 */
function openEditWin(pPaySettingID, pUserKeyIDWin, pPayTypeNameWin, pNotifyUrlWin, pReturnUrlWin, pQuitUrlWin) {

    mPaySettingID = pPaySettingID;
    ...
    //窗口赋值
    $("#UserKeyIDWin").html(pUserKeyIDWin);
    $("#PayTypeNameWin").html(pPayTypeNameWin);
    $("#NotifyUrlWin").val(pNotifyUrlWin);
    $("#ReturnUrlWin").val(pReturnUrlWin);
    $("#QuitUrlWin").val(pQuitUrlWin);
...
function savePaySetting() {
    ...
    //获取表单值
    var NotifyUrlWin = $("#NotifyUrlWin").val().trim();
    var ReturnUrlWin = $("#ReturnUrlWin").val().trim();
    var QuitUrlWin = $("#QuitUrlWin").val().trim();

    if (NotifyUrlWin == "" || ReturnUrlWin == "" || QuitUrlWin == "") {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "2", "PaySettingID": mPaySettingID, "NotifyUrl": NotifyUrlWin, "ReturnUrl": ReturnUrlWin, "QuitUrl": QuitUrlWin,
    };
