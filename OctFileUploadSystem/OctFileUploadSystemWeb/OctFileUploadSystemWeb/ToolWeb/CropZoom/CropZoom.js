...
//Ajax请求路径 
var ajaxUrl = "../../../../ToolWeb/CropZoom/CropZoom.ashx";
...
var mFileRelativePath = ""; //裁剪图片的相对路径
...
$(function () {
    ...
    mFileRelativePath = $("#hidFileRelativePath").val().trim();
...
function initCropZoom() {
    ...
    $('.send_data_right').click(function () {
        ...
        limitCropNum++;
        //裁剪提示
        $(".send_data_right").html("…裁剪中…");

        //cropzoom.send('ProcessImage.ashx', 'POST', { ID: 11 }, function (imgRet) {
        cropzoom.send(ajaxUrl, 'POST', { FileRelativePath: mFileRelativePath }, function (imgRet) {
            if (imgRet.indexOf('Error') >= 0) {
                ...
                alert("【裁剪成功】请刷新相应图片,查看效果！");

                var hidCropRedirectURL = $("#hidCropRedirectURL").val().trim();

                if (hidCropRedirectURL != "" && hidCropRedirectURL != "#") {
                    //跳转到URL
                    window.location.href = hidCropRedirectURL;
                }
                else {
