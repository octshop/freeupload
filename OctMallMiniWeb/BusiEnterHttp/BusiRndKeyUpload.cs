...
using HttpServiceNS;
using PublicClassNS;
using SendUploadFileHttpRemoteNS;
using System;
...
/// <summary>
/// 处理【RndKey与上传文件或图片】的相关逻辑  具体是保存在RndKeyRsa.config配置文件中
/// </summary>
namespace BusiRndKeyUploadNS
{
    public class BusiRndKeyUpload
    {
        ...
        public static string getRndKeyRsa(string pOneTime = "false")
        {
            ...
            else //没有超时
            {
                ...
                if (_appSettingsValue.IndexOf("超时") >= 0 || _appSettingsValue.IndexOf("错误") >= 0 || _appSettingsValue.IndexOf("无效") >= 0 || _appSettingsValue.Length < 40)
                {
                    //重新设置只在 RndKeyRsa 值
                    return resetRndKeyRsa(pOneTime);
                }
        ...
        #region 【带上传文件的  RndKey和SignVal值 Http请求封装】

        /// <summary>
        /// 带RndKeyRSA的Http上传文件请求 无需进入用户登录验证的操作 【在上传处理页面直接调用此方法】
        /// </summary>
        /// <param name="pHttpUrl">API请求的URL [http://192.168.1.220:1200/Test/VerifyApiNoLogin]</param>
        /// <param name="pVerifyType">Http验证类型</param>
        /// <param name="pPostParamDic">额外传递的POST参数</param>
        /// <returns></returns>
        public static string httpNoLoginRndKeyRsaUpload(string pHttpUrl, string pVerifyType, Dictionary<string, string> pPostParamDic)
        {
            //得到RSA加密后的RndKey
            string _rndKeyRSA = getRndKeyRsa();
            ...
            //发送Http请求
            string _backMsg = SendUploadFileHttpRemote.sendUploadFileRemote(pHttpUrl, _dic);
            return _backMsg;
        }


        /// <summary>
        ///  带SignVal的Http上传文件请求 需要进入用户登录验证的操作 【在上传处理页面直接调用此方法】
        /// </summary>
        /// <param name="pHttpUrl">API请求的URL [http://192.168.1.220:1200/Test/VerifyApiNoLogin]</param>
        /// <param name="pVerifyType">Http验证类型</param>
        /// <param name="pUserID">用户UserID</param>
        ...
        /// <param name="pPostParamDic">额外传递的POST参数</param>
        /// <returns></returns>
        public static string httpLoginSignValUpload(string pHttpUrl, string pVerifyType, string pUserID, string pLoginPwd, string pExtraDataArr, Dictionary<string, string> pPostParamDic)
        {
            //得到RSA加密后的RndKey
            string _rndKeyRSA = getRndKeyRsa();
