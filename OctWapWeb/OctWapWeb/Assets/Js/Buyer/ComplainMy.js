...
/**-----定义公共变量------**/
var mAjaxUrl = "../BuyerAjax/ComplainMy";
...
function pageSuccess(curPageDataJson) {
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        if (dataPage.ComplainCategory == "order") {
            ComplainCategory = "订单";
        }
        else if (dataPage.ComplainCategory == "aftersale") {
            ComplainCategory = "售后";
        }
        else if (dataPage.ComplainCategory == "PresentOrder") {
            ComplainCategory = "礼品订单";
        }
        else if (dataPage.ComplainCategory == "shop") {
            ComplainCategory = "商家";
        }
        else if (dataPage.ComplainCategory == "paltform") {
            ComplainCategory = "平台";
        }

        myJsVal += "<li class=\"complain-item\" onclick=\"window.location.href=\'../Buyer/ComplainDetail?CID=" + dataPage.ComplainID +"\'\">";        myJsVal += "  <div class=\"complain-item-top\">";        myJsVal += "      <div class=\"complain-item-left\">";        myJsVal += "          投诉ID：" + dataPage.ComplainID + " &nbsp; 类别：" + ComplainCategory +"";        myJsVal += "      </div>";        myJsVal += "      <div class=\"complain-item-right\">";        myJsVal += "" + dataPage.ComplainStatus +"";        myJsVal += "      </div>";        myJsVal += "  </div>";        myJsVal += "  <div class=\"complain-item-mid\">";        myJsVal += "      <b>配送问题：</b>" + dataPage.ComplainContent;        myJsVal += "  </div>";        myJsVal += "  <div class=\"complain-item-bottom\">";        myJsVal += "      <div class=\"complain-bottom-left\">";        myJsVal += "" + dataPage.WriteDate +"";        myJsVal += "      </div>";        myJsVal += "      <div class=\"complain-bottom-left\">";        //投诉状态( 商家处理  -- 官方介入 --  买家确认 --  完成  )   如果官方介入了，就可以官方填写回复后，直接完成投诉。不需要买家再确认        if (dataPage.ComplainStatus == "商家处理") {            myJsVal += "<input type=\"button\" value=\"申请官方介入\" onclick=\"applyOfficialIntervene('" + dataPage.ComplainID +"')\" />";
        }        else if (dataPage.ComplainStatus == "官方介入") {
        }        else if (dataPage.ComplainStatus == "买家确认") {            myJsVal += "<input type=\"button\" value=\"申请官方介入\" style=\"margin-right:10px;\" onclick=\"applyOfficialIntervene('" + dataPage.ComplainID + "')\" /><input type=\"button\" value=\"确认投诉\" onclick=\"buyerConfirmComplain('" + dataPage.ComplainID +"')\" />";
        }        else if (dataPage.ComplainStatus == "完成") {
        }         myJsVal += "      </div>";        myJsVal += "  </div>";        myJsVal += "</li>";
 ...
 * 申请官方介入
 * */
function applyOfficialIntervene(pComplainID) {
    ...
    confirmWin("确认申请官方介入吗？", function () {
        ...
        //构造POST参数
        var dataPOST = {
            "Type": "2", "ComplainID": pComplainID,
        };
        console.log(dataPOST);
        //正式发送异步请求
        $.ajax({
            type: "POST",
            url:  "../BuyerAjax/ComplainDetail?rnd=" + Math.random(),
            data: dataPOST,
            dataType: "html",
            success: function (reTxt, status, xhr) {
 ...
 * 买家确认投诉，投诉完成
 * */
function buyerConfirmComplain(pComplainID) {
