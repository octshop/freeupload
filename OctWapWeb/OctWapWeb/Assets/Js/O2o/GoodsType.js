...
//AjaxURL
var mAjaxUrl = "../O2oAjax/GoodsType";

var mBuyerUserID = ""; //买家UserID

var mGoodsTypeIDThird = ""; //第三级商品类目ID
var mGoodsTypeIDSec = ""; //第三级商品类目
...
$(function () {
    ...
    mGoodsTypeIDThird = $("#hidGoodsTypeIDThird").val().trim();

    //加载商品搜索条件选项卡信息O2O
    loadGoodsSearchWhereTabMsg(function () {
 ...
 * 加载商品搜索条件选项卡信息O2O
 * */
function loadGoodsSearchWhereTabMsg(pCallBack) {
    //构造POST参数
    var dataPOST = {
        "Type": "2", "GoodsTypeIDThird": mGoodsTypeIDThird,
    };
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            console.log("加载商品搜索条件选项卡信息O2O=" + reTxt);
            if (reTxt != "") {
                var _jsonReTxt = JSON.parse(reTxt);

                //--------商品分类-------//
                var GoodsTypeEntityList = _jsonReTxt.GoodsTypeEntityList;
                if (GoodsTypeEntityList.GoodsTypeSecName != "" && GoodsTypeEntityList.GoodsTypeSecName != null) {

                    $("#GoodsTypeTtitleB").html(GoodsTypeEntityList.GoodsTypeSecName);
                    $("#SearchWhere1Span").html(GoodsTypeEntityList.GooGoodsTypeCurrent.GoodsTypeName);

                    var xhtmlAllWhereType1 = "";
                    xhtmlAllWhereType1 += "<span class=\"goods-type-item\" onclick=\"clickItemSearchWhere1('','" + GoodsTypeEntityList.GoodsTypeIDSec + "')\">全部分类</span>";

                    xhtmlAllWhereType1 += "<span class=\"goods-type-item goods-type-current\" onclick=\"clickItemSearchWhere1('" + GoodsTypeEntityList.GooGoodsTypeCurrent.GoodsTypeID + "')\">";
                    xhtmlAllWhereType1 += "" + GoodsTypeEntityList.GooGoodsTypeCurrent.GoodsTypeName + "";
                    xhtmlAllWhereType1 += "</span>";

                    for (var i = 0; i < GoodsTypeEntityList.GooGoodsTypeList.length; i++) {
                        xhtmlAllWhereType1 += "<span class=\"goods-type-item\" onclick=\"clickItemSearchWhere1('" + GoodsTypeEntityList.GooGoodsTypeList[i].GoodsTypeID + "')\">" + GoodsTypeEntityList.GooGoodsTypeList[i].GoodsTypeName + "</span>";
                    }
                    //显示代码插入前台
                    $("#AllWhereType1").html(xhtmlAllWhereType1);
                }

                //--------全部地区-------//
                if (_jsonReTxt.UserCountyList != undefined && _jsonReTxt.UserCountyList != null) {
                    var UserCountyList = _jsonReTxt.UserCountyList;
...
function searchWhere3() {
    ...
    mIsDiffTab = "searchWhere3";
}

/**
 * 选择商品分类
 * @param {any} pGoodsTypeIDThird 第三级商品类目
 */
function clickItemSearchWhere1(pGoodsTypeIDThird, pGoodsTypeIDSec = "") {


    mGoodsTypeIDThird = pGoodsTypeIDThird;
    mGoodsTypeIDSec = pGoodsTypeIDSec;
...
function initSearchWhereArr() {
    ...
    // RegionCodeArr + "^" + pPageOrderName
    //var mSearchKeyword = "";
    mSearchWhereArr = mRegionCountyCode + "^" + mPageOrderName;
...
function pageSuccess(curPageDataJson) {
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        if (dataPage.Discount > 0) {
            _bageSpan = "<span>" + dataPage.Discount + "折</span>";
        }
        if (dataPage.CountSecKill > 0) {
            _bageSpan = "<span>秒杀</span>";
        }
        if (dataPage.GroupMsgCount > 0) {
            _bageSpan = "<span>团购</span>";
        }
