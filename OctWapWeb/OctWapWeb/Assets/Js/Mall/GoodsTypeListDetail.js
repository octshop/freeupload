...
//AjaxURL
var mAjaxUrl = "../MallAjax/GoodsTypeListDetail";
...
$(function () {
    ...
    //是否加载所有
    var hidLoadAll = $("#hidLoadAll").val().trim();
 ...
 * 切换分类显示层
 * */
function tglAllTypeShow() {

    var _styleDisplay = $("#AllTypeShow").get(0).style.display;

    if (_styleDisplay == "none") {
        $("#AllTypeShow").show();
        $("#BtnArrowImg").attr("src", "../Assets/Imgs/Icon/arrowup.png");
    }
    else {
        $("#AllTypeShow").hide();
        $("#BtnArrowImg").attr("src", "../Assets/Imgs/Icon/arrowdown.png");
    }
}
 ...
 * 根据第三级商品类目ID,加载所有同级的商品类目信息
 * */
function loadListGoodsTypeThirdByThird(pCallBack) {

    //构造POST参数
    var dataPOST = {
        "Type": "3", "GoodsTypeIDThird": mGoodsTypeIDThird,
    };
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            console.log("根据第三级商品类目ID,加载所有同级的商品类目信息=" + reTxt);
            if (reTxt != "") {
                var _jsonReTxt = JSON.parse(reTxt);

                var GoodsTypeSecName = _jsonReTxt.GoodsTypeSecName;

                mGoodsTypeIDSec = _jsonReTxt.GoodsTypeIDSec;


                var GooGoodsTypeCurrent = _jsonReTxt.GooGoodsTypeCurrent;
                var GooGoodsTypeList = _jsonReTxt.GooGoodsTypeList;

                var myJsVal = "";
                myJsVal += "<li class=\"goods-type-li goods-type-li-all\" onclick=\"chgAllGoodsType()\">";
                myJsVal += "    <div>全部</div>";
                myJsVal += "</li>";
                myJsVal += "<li onclick=\"chgThirdGoodsTypeItem('" + GooGoodsTypeCurrent.GoodsTypeID + "')\">";
                myJsVal += "    <div class=\"goods-type-li goods-type-li-current\">" + GooGoodsTypeCurrent.GoodsTypeName + "</div>";
                myJsVal += "</li>";

                if (GooGoodsTypeList != undefined && GooGoodsTypeList != null) {
                    for (var i = 0; i < GooGoodsTypeList.length; i++) {
                        myJsVal += "<li onclick=\"chgThirdGoodsTypeItem('" + GooGoodsTypeList[i].GoodsTypeID + "')\">";
                        myJsVal += "    <div class=\"goods-type-li\">" + GooGoodsTypeList[i].GoodsTypeName + "</div>";
                        myJsVal += "</li>";
                    }
                }
                ...
                $("#AllTypeShowUl").html(myJsVal);
                //页标题
                $("#GoodsTypeSecNameB").html(GoodsTypeSecName);
 ...
 * 全部分类商品
 * */
function chgAllGoodsType() {

    $(".goods-type-li").removeClass("goods-type-li-current");
    $(".goods-type-li-all").addClass("goods-type-li-current");
 ...
 * 切换第三级分类商品
 * */
function chgThirdGoodsTypeItem(pGoodsTypeIDThird) {
...
function pageSuccess(curPageDataJson) {
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        if (dataPage.GoodsTitle.length > 23) {
            dataPage.GoodsTitle = dataPage.GoodsTitle.substring(0, 23) + "...";
        }
