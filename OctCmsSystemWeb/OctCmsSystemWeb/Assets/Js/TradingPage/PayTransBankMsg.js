...
//AjaxURL
var mAjaxUrl = "../Trading/PayTransBankMsg";
...
var searchContent = function () {
    ...
    var TransBankID_se = $("#TransBankID_se").val().trim();
    var ShopUserID_se = $("#ShopUserID_se").val().trim();
    var BankName_se = $("#BankName_se").val().trim();
    var BankAccName_se = $("#BankAccName_se").val().trim();
    var OpenAccBank_se = $("#OpenAccBank_se").val().trim();
    var BankAccount_se = $("#BankAccount_se").val().trim();
    var TransType_se = $("#TransType_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "TransBankID": TransBankID_se, "ShopUserID": ShopUserID_se, "BankName": BankName_se, "BankAccName": BankAccName_se,
        "OpenAccBank": OpenAccBank_se, "BankAccount": BankAccount_se, "TransType": TransType_se, "IsLock": IsLock_se, "WriteDate": WriteDate_se,
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += " <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.TransBankID + "\" /></td>";
        myJsVal += " <td>" + indexDataPage.TransBankID + "</td>";
        myJsVal += " <td>" + indexDataPage.ShopUserID + "<i style=\"color: #939393;display: block\"><a href=\"#\" target=\"_blank\">" + indexDataPageExtra.ShopName + "</a></i></td>";
        myJsVal += " <td>" + indexDataPage.BankName + "</td>";
        myJsVal += " <td>" + indexDataPage.OpenAccBank + "</td>";
        myJsVal += " <td>" + indexDataPage.BankAccount + "</td>";
        myJsVal += " <td>" + indexDataPageExtra.TransTypeName + "(" + indexDataPage.TransType + ")</td>";
        ...
        if (indexDataPage.IsLock == "false") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"lockPayTransBankMsg('" + indexDataPage.TransBankID + "')\">锁定</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"lockPayTransBankMsg('" + indexDataPage.TransBankID + "')\">解锁</button>";
        }
        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"openEditWin('" + indexDataPage.TransBankID + "', '" + indexDataPage.TransType + "', '" + indexDataPage.ShopUserID + "', '" + indexDataPage.BankName + "', '" + indexDataPage.BankAccName + "', '" + indexDataPage.OpenAccBank + "', '" + indexDataPage.BankAccount + "')\" >修改</button>";
        myJsVal += " </td>";
        myJsVal += "</tr>";
 ...
 * 批量删除 信息
 * */
function delPayTransBankMsgArr() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "4", "TransBankIDArr": _selIDArr,
    };
...
/**
 * 解锁锁定信息
 * @param {any} pTransBankID
 */
function lockPayTransBankMsg(pTransBankID) {

    //构造POST参数
    var dataPOST = {
        "Type": "3", "TransBankID": pTransBankID,
    };
    console.log(dataPOST);
 ...
 */
function openEditWin(pTransBankID, pTransTypeWin, pShopUserIDWin, pBankNameWin, pBankAccNameWin, pOpenAccBankWin, pBankAccountWin) {

    mTransBankID = pTransBankID;
    ...
    //初始化窗口
    $("#TransTypeWin").val(pTransTypeWin);
    $("#ShopUserIDWin").val(pShopUserIDWin);
    $("#ShopUserIDWin").attr("disabled", true);
    $("#BankNameWin").val(pBankNameWin);
    $("#BankAccNameWin").val(pBankAccNameWin);
    $("#OpenAccBankWin").val(pOpenAccBankWin);
    $("#BankAccountWin").val(pBankAccountWin);
}

/**
 * 提交信息
 * */
function submitPayTransBankMsg() {

    //获取表单值
    var TransTypeWin = $("#TransTypeWin").val().trim();
    var ShopUserIDWin = $("#ShopUserIDWin").val().trim();
    var BankNameWin = $("#BankNameWin").val().trim();
    var BankAccNameWin = $("#BankAccNameWin").val().trim();
    var OpenAccBankWin = $("#OpenAccBankWin").val().trim();
    var BankAccountWin = $("#BankAccountWin").val().trim();

    //---判断输入是否合法-----//
    if (ShopUserIDWin == "" || BankNameWin == "" || BankAccNameWin == "" || OpenAccBankWin == "" || BankAccountWin == "") {
        toastWinToDiv("所有项不能为空！", "dragWinDiv");
        return;
    }

    //构造POST参数
    var dataPOST = {
        "Type": "2", "TransBankID": mTransBankID, "TransType": TransTypeWin, "ShopUserID": ShopUserIDWin, "BankName": BankNameWin, "BankAccName": BankAccNameWin, "OpenAccBank": OpenAccBankWin, "BankAccount": BankAccountWin,
    };
    console.log(dataPOST);
