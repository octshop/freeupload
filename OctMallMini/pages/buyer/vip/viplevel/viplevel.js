// pages/buyer/vip/viplevel/viplevel.js
...
Page({
  ...
  data: {
    ...
    loadExplainTextList_Data: null, //加载指定类型的说明性文本
   ...
   * 加载指定类型的说明性文本
   */
  loadExplainTextList: function () {
    //构造POST参数
    var _dataPOST = {
      "SignKey": mHttp.signKeyRsa(app.globalData.loginBuyerUserID, app.globalData.loginPwdSha1),
      "Type": "1",
      "ExplainType": "VipLevel^CreditScore",
    };

    //正式发送Http请求
    mHttp.postHttp(app.apiURLData.vipApi_VipLevel, _dataPOST, _jsonReTxt => {
      if (_jsonReTxt == "" || _jsonReTxt == undefined || _jsonReTxt == null) {
        return;
      }
      ...
      //设置公共变量值
      this.setData({
        loadExplainTextList_Data: _jsonReTxt
      });
    });
  },
