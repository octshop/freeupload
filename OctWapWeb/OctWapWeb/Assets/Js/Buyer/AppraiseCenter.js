...
/**-----定义公共变量------**/
var mAjaxUrl = "../BuyerAjax/AppraiseCenter";

var mOrderStatus = "待评价"; //"完成"; //"待评价"; //当前订单状态
...
function getListDataFromNet(pageNum, pageSize, successCallback, errorCallback) {
    ...
    var dataGET = {
        ...
        "IsLoadFieldName": "SumAppraiseIntegral",
    };
    //正式发送GET请求
    $.ajax({
        type: "POST",
        url: "../BuyerAjax/MyOrder?rnd=" + Math.random(),
        data: dataGET,
        dataType: "html",
...
/**
 * 切换当前评价信息 选项卡
 * @param {any} pOrderStatus "完成"; //"待评价"; //当前订单状态
 */
function chgTabAppraise(pOrderStatus) {

    mOrderStatus = pOrderStatus; //"完成"; //"待评价"; //当前订单状态

    $("#SubTab_1").removeClass("oct-header-sub-curren-a");
    $("#SubTab_2").removeClass("oct-header-sub-curren-a");
    if (pOrderStatus == "待评价") {
        $("#SubTab_1").addClass("oct-header-sub-curren-a");
    }
    else if (pOrderStatus == "完成") {
        $("#SubTab_2").addClass("oct-header-sub-curren-a");
    }
 ...
 * 统计买家的评价信息
 * */
function initCountBuyerAppraiseMsg() {
    ...
    $.ajax({
        ...
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("统计买家的评价信息=" + reTxt);
            if (reTxt != "") {
                var _jsonReTxt = JSON.parse(reTxt);
                $("#CountWaitAppraisedOrder").html(_jsonReTxt.CountWaitAppraisedOrder);
                $("#CountAppraisedOrder").html(_jsonReTxt.CountAppraisedOrder);
            }
        }
    });
