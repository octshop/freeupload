...
/**-----定义公共变量------**/
//AjaxURL
var mAjaxUrl = "../Trading/BuyerRecharge";
...
var searchContent = function () {
    ...
    var RechargeID_se = $("#RechargeID_se").val().trim();
    var SerialNumber_se = $("#SerialNumber_se").val().trim();
    var RechargeAmt_se = $("#RechargeAmt_se").val().trim();
    var BuyerUserID_se = $("#BuyerUserID_se").val().trim();
    var RechargeMemo_se = $("#RechargeMemo_se").val().trim();
    var RechargeStatus_se = $("#RechargeStatus_se").val().trim();
    var FromType_se = $("#FromType_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "RechargeID": RechargeID_se, "SerialNumber": SerialNumber_se, "RechargeAmt": RechargeAmt_se, "BuyerUserID": BuyerUserID_se, "RechargeMemo": RechargeMemo_se, "RechargeStatus": RechargeStatus_se, "FromType": FromType_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += " <tr>";
        myJsVal += " <td>" + indexDataPage.RechargeID + "</td>";
        myJsVal += " <td>" + indexDataPage.SerialNumber + "</td>";
        myJsVal += " <td>" + indexDataPage.RechargeAmt + "</td>";
        myJsVal += " <td>" + indexDataPage.RechargeStatus + "</td>";
        myJsVal += " <td>" + indexDataPage.FromType + "<br />" + indexDataPageExtra.FromTypeName + "</td>";
        myJsVal += " <td>" + indexDataPage.RechargeMemo + "</td>";
 ...
 * 删除一天之后,没有充值成功的信息
 * */
function delBuyerNoRechargeMsg() {


    confirmWinWidth("确认定要删除吗？", function () {
        ...
        $.ajax({
            ...
            dataType: "html",
            success: function (reTxt, status, xhr) {
                console.log("删除一天之后,没有充值成功的信息=" + reTxt);
                if (reTxt != "") {
