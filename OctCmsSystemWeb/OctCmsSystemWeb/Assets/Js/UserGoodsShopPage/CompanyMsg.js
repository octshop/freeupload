...
//AjaxURL
var mAjaxUrl = "../UserGoodsShop/CompanyMsg";
...
var searchContent = function () {
    ...
    var CreditCode_se = $("#CreditCode_se").val().trim();
    var CompanyName_se = $("#CompanyName_se").val().trim();
    var RegAddress_se = $("#RegAddress_se").val().trim();
    var LegalPerson_se = $("#LegalPerson_se").val().trim();
    var BusiScope_se = $("#BusiScope_se").val().trim();
    var RegMoney_se = $("#RegMoney_se").val().trim();
    var ShopUserID_se = $("#ShopUserID_se").val().trim();
    var IsCheck_se = $("#IsCheck_se").val().trim();
    var IsLock_se = $("#IsLock_se").val().trim();
    var SetUpDate_se = $("#SetUpDate_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "CompanyID": CompanyID_se, "CreditCode": CreditCode_se, "CompanyName": CompanyName_se,
        "RegAddress": RegAddress_se, "LegalPerson": LegalPerson_se, "BusiScope": BusiScope_se,
        "RegMoney": RegMoney_se, "ShopUserID": ShopUserID_se, "IsCheck": IsCheck_se,
        "IsLock": IsLock_se, "SetUpDate": SetUpDate_se, "WriteDate": WriteDate_se
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";        myJsVal += "   <td>" + indexDataPage.CompanyID + "</td>";        myJsVal += "   <td><a href=\"../UserGoodsShopPage/ShopMsgDetail?UserID=" + indexDataPage.ShopUserID + "\" target=\"_blank\">" + indexDataPage.ShopUserID + "</a></td>";        myJsVal += "   <td><a href=\"../UserGoodsShopPage/CompanyMsgDetail?CompanyID=" + indexDataPage.CompanyID + "\" target=\"_blank\">" + indexDataPage.CompanyName + "</a></td>";        myJsVal += "   <td>" + indexDataPage.LegalPerson + "</td>";        myJsVal += "   <td>" + indexDataPage.CreditCode + "</td>";        myJsVal += "   <td>" + indexDataPage.RegMoney + "</td>";        myJsVal += "   <td>";        myJsVal += "   " + _IsCheck + "";        myJsVal += "   </td>";        myJsVal += "   <td>" + indexDataPage.IsLock + "</td>";        myJsVal += "   <td>" + indexDataPage.WriteDate + "</td>";        myJsVal += "   <td>";        //审核
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"openEditWin(" + indexDataPage.CompanyID + ")\"><span class=\"";
        if (indexDataPage.IsCheck == "true") {
        ...
        //编辑
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"window.location.href='../UserGoodsShopPage/CompanyMsgEdit?CompanyID=" + indexDataPage.CompanyID + "'\"><span class=\"am-icon-pencil-square-o";
        myJsVal += "\"></span></button>"

        //锁定
        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning  am-round\" onclick=\"lockCompanyMsg(" + indexDataPage.CompanyID + ")\"><span id=\"BtnToggleLock\" class=\"";
...
function openEditWin(pMsgID) {
    ...
    //打开Dialog弹出窗口
    openDialogWinNoClose("审核公司信息", mAddEditWinHtml, function () {
 ...
 */
function initCompanyMsg(pMsgID) {

    //构造POST参数
    var dataPOST = {
        "Type": "3", "CompanyID": pMsgID,
    };
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                $("#CompanyID_ae").html("" + _jsonReTxt.CompanyID + " ，" + _jsonReTxt.CompanyName + "");
...
function aeMsg(pExeType) {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "4", "CompanyID": mMsgID, "IsCheck": IsCheck_ae, "CheckReason": CheckReason_ae,
    };
    console.log(dataPOST);
 ...
 * @param {any} pMsgID
 */
function lockCompanyMsg(pMsgID) {


    //构造POST参数
    var dataPOST = {
        "Type": "5", "CompanyID": pMsgID,
    };
    console.log(dataPOST);
