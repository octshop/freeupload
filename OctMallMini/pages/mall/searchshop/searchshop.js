// pages/mall/searchshop/searchshop.js
//==============引入相关的Js文件========//
...
Page({
  ...
  data: {
    ...
    loadSearchHistoryShop_Data: null, //加载买家店铺搜索历史
   ...
   * 加载买家店铺搜索历史
   */
  loadSearchHistoryShop: function () {
    ...
    //正式发送Http请求
    mHttp.postHttp(app.apiURLData.searchApi_SearchShop, _dataPOST, _jsonReTxt => {
      ...
      //设置公共变量值
      this.setData({
        loadSearchHistoryShop_Data: _jsonReTxt,
      });
  ...
  clickSearchShop: function (e) {
    ...
    mUtils.logOut("搜索类别=" + this.data.searchType);
    if (this.data.searchType == "Entity") {
      //跳转到店铺搜索结果页
      mUtils.navigateToURL("../../../pages/mall/searchshopresulto2o/searchshopresulto2o?SC=" + _searchTxt);
    } else {
      //跳转到店铺搜索结果页
      mUtils.navigateToURL("../../../pages/mall/searchshopresult/searchshopresult?SC=" + _searchTxt);
    }
