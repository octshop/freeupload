...
$(function () {
    ...
    //初始化商品信息
    initGoodsMsg(function () {

        setTimeout(function () {
            //为编辑器赋值
            mEditor.insertHtml(mEditorContent);
        }, 2000);
 ...
 */
function initGoodsMsg(pCallBack) {
    ...
    $.ajax({
        type: "POST",
        url: "../Goods/GoodsEdit?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                //运费模板
                mFtID = _jsonBack.GooGoodsMsg.FtID;

                //赋值到页面窗体
                setValToForm(_jsonBack);
...
/**
 * 赋值到页面窗体
 * @param {any} pJsonInitMsg 初始化信息 Json对象
 */
function setValToForm(pJsonInitMsg) {

    var _gooGoodsMsg = pJsonInitMsg.GooGoodsMsg;

    $("#hidShopUserID").val(_gooGoodsMsg.ShopUserID);
    //商家UserID
    mUserID = _gooGoodsMsg.ShopUserID;

    //商品标题赋值
    $("#GoodsTitle").val(_gooGoodsMsg.GoodsTitle);

    //商品描述
    mEditorContent = _gooGoodsMsg.GoodsDesc;

    //分销
    if (_gooGoodsMsg.IsShareGoods == "true") {
        $("#IsShareGoods").prop("checked", true);
    }
    if (_gooGoodsMsg.IsShowShareMoney == "true") {
        $("#IsShowShareMoney").prop("checked", true);
    }

    if (_gooGoodsMsg.MarketPrice != 0) {
        $("#MarketPrice").val(_gooGoodsMsg.MarketPrice);
    }
    if (_gooGoodsMsg.Discount != 0) {
        $("#Discount").val(_gooGoodsMsg.Discount);
    }


    //设置商品规格属性标题
    mSpecTitle = pJsonInitMsg.GooSpecParamName.SpecTitle;
    mPropTitle = pJsonInitMsg.GooSpecParamName.SpecAttrName;

    //显隐规格属性内容
    if (_gooGoodsMsg.IsSpecParam == "true") {
        ...
        if (_gooGoodsMsg.GoodsPrice != "0") {
            $("#GoodsPrice").val(_gooGoodsMsg.GoodsPrice);
        }
        else {
            $("#GoodsPrice").val("");
        }
        if (_gooGoodsMsg.StockNum != "0") {
            $("#StockNum").val(_gooGoodsMsg.StockNum);
        }
        else {
            $("#StockNum").val();
        }
    ...
    //其他设置
    if (_gooGoodsMsg.IsShopExpense == "true") {
        $("#IsShopExpense").prop("checked", true);
        $("#IsExpressHome").prop("checked", false);
    }
    else if (_gooGoodsMsg.IsShopExpense == "false") {
        $("#IsShopExpense").prop("checked", false);
        $("#IsExpressHome").prop("checked", true);
    }
    else if (_gooGoodsMsg.IsShopExpense == "both") {
        $("#IsShopExpense").prop("checked", true);
        $("#IsExpressHome").prop("checked", true);
    }

    //只有实体店才支持到店消费或自取
    if (pJsonInitMsg.ShopMsg.IsEntity == "false") {
        $("#IsShopExpense").prop("checked", false);
        ...
        if (_gooGoodsMsg.IsOfflinePay == "true") {
            $("#IsOfflinePay").prop("checked", true);
        }
    }


    //支付货到付款
    if (_gooGoodsMsg.IsPayDelivery == "true") {
        $("#IsPayDelivery").prop("checked", true);
    }
    //包装说明 
    $("#PackAfterSaleDesc").val(_gooGoodsMsg.PackAfterSaleDesc);

    //初始化商品类别信息
    initGoodsTypeMsg(pJsonInitMsg.GooGoodsTypeList);

    //加载类目必填属性
    initGoodsTypeNeedProp(function () {

        //初始化商品类目必填属性
        initGoodsTypeNeedPropVal(_gooGoodsMsg.GoodsTypeNeedProp);

    });

    //初始化加载 类目自定义属性
    if (_gooGoodsMsg.GoodsTypeCustomProp != "") {
        initGoodsTypeCustomProp(_gooGoodsMsg.GoodsTypeCustomProp);
    }
    ...
    loadShopGoodsType(function () {
        ...
        //初始化店铺商品类别
        initShopGoodsType(pJsonInitMsg.ShopGoodsTypeList);

    });

    //初始化商品照片上传 插件
    initGoodsImgUploadList(pJsonInitMsg.GoodsImgList);
    ...
    //初始化商品赠品 信息
    initGiftMsg(pJsonInitMsg.GooGiftMsgList);
}

/**
 * 初始化商品赠品 信息
 * @param pJsonGooGiftMsgList 商品赠品信息 Json对象
 * */
function initGiftMsg(pJsonGooGiftMsgList) {

    if (pJsonGooGiftMsgList == undefined || pJsonGooGiftMsgList == "" || pJsonGooGiftMsgList == null) {
        return;
    }

    var _giftNumList = pJsonGooGiftMsgList.GiftNumList;
    var _giftMsgList = pJsonGooGiftMsgList.GiftMsgList;

    //构造初始化需要的变量
    mJsonGooGiftMsgList = _giftMsgList;

    //构造mSelGiftNumArr和mSelGiftIDArr数组
    for (var i = 0; i < _giftNumList.length; i++) {
        mSelGiftNumArr += _giftNumList[i].GiftNum + "^";
        mSelGiftIDArr += _giftMsgList[i].GiftID + "^";
    }
    //去掉前后“^”
    ...
    //构造已选择的赠品列表显示代码
    var _xhtml = xhtmlSelGiftList();
    //console.log("GoodsGiftUL=" + _xhtml);
    //重新构造显示代码
    $("#GoodsGiftUL").html(_xhtml);
}

/**
 * 初始化 商品规格属性信息
 * */
function initGooSpecParam() {
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                //规格属性价格库存 Json对象列表
                mJsonArrSpecProp = _jsonBack;

                //构造规格属性页面显示代码
                var xhtmlSpecPropMsgArr = xhtmlSpecPropMsgInit(_jsonBack).split("^");
                //显示代码插入前台 
                ...
                //选择第一个规格信息
                chgTabSpecInit(mJsonArrSpecProp[0].SpecID, true);
 ...
 */
function xhtmlSpecPropMsgInit(pJsonArrSpecProp) {
    ...
    for (var i = 0; i < pJsonArrSpecProp.length; i++) {
        ...
        if (_specMsg.SpecParamImg != "") {
            _specXhtml += "<div name=\"SpecListName\" data-price-stock=\"" + _specMsg.GoodsPrice + "|" + _specMsg.StockNum + "\" onclick=\"chgTabSpecInit(" + _specMsg.SpecID + ")\" class=\"" + _currentClass + "\"><img src=\"//" + _specMsg.SpecParamImg + "\" /></div>";
        }
        else {
            _specXhtml += "<div name=\"SpecListName\" data-price-stock=\"" + _specMsg.GoodsPrice + "|" + _specMsg.StockNum + "\" onclick=\"chgTabSpecInit(" + _specMsg.SpecID + ")\" class=\"" + _currentClass + "\"><span>" + _specMsg.SpecParamVal + "</span></div>";
        }

        //规格下面的属性信息数组
        var _propArr = _specMsg.PropList;
        //console.log(_propArr);

        var _currentStyleDisplay = "none";
        if (i == 0) {
            _currentStyleDisplay = "normal";
        }
        _propXhtml += " <div id=\"PropList_" + _specMsg.SpecID + "\" name=\"PropListName\" class=\"price-spec-div-item\" style=\"display:" + _currentStyleDisplay + "\">";
        ...
        for (var j = 0; j < _propArr.length; j++) {
            ...
            //console.log("_propMsg.SpecID=" + _propMsg.SpecID);

            if (_propMsg.SpecParamImg != "") {
                _propXhtml += "<div name=\"PropItemName\" id=\"PropItem_" + _propMsg.SpecID + "\" onclick=\"chgPropTablInit(" + _specMsg.SpecID + "," + _propMsg.SpecID + ")\" data-price-stock=\"" + _propMsg.GoodsPrice + "|" + _propMsg.StockNum + "\" class=\"" + _currentClass + "\"><img src=\"//" + _propMsg.SpecParamImg + "\" /></div>";
            }
            else {
                _propXhtml += "<div name=\"PropItemName\" id=\"PropItem_" + _propMsg.SpecID + "\" onclick=\"chgPropTablInit(" + _specMsg.SpecID + "," + _propMsg.SpecID + ")\" data-price-stock=\"" + _propMsg.GoodsPrice + "|" + _propMsg.StockNum + "\" class=\"" + _currentClass + "\"><span>" + _propMsg.SpecParamVal + "</span></div>";
            }
        }
        _propXhtml += " </div>";
    }
    //alert("_specXhtml=" + _specXhtml);
    //console.log("_propXhtml=" + _propXhtml);
 ...
 */
function chgTabSpecInit(pSpecID, pIsLoading) {
    ...
    //console.log("初始化切换mJsonArrSpecProp" + mJsonArrSpecProp);
    //重新加载属性Xhtml
    var xhtmlSpecPropMsgArr = xhtmlSpecPropMsgInit(mJsonArrSpecProp).split("^");
    //显示代码插入前台 
    $("#PropItemDiv").html(xhtmlSpecPropMsgArr[1]);
    //console.log("PropItemDiv=" + xhtmlSpecPropMsgArr[1]);
    ...
    //显示当前的属性Tab
    $("#PropList_" + pSpecID).show();

    //console.log("#PropList_" + pSpecID);

    //切换Tab时的价格与库存显示
    var _propItemNameArr = $("#PropList_" + pSpecID + " div[name='PropItemName']");
...
/**
 * 切换属性选项卡 --初始化
 * @param {any} pSpecID 规格ID
 * @param {any} pPropID 属性ID (字段 SpecIDPre)
 */
function chgPropTablInit(pSpecID, pPropID) {
    ...
    //设置当前选择的Tab样式
    $("#PropItem_" + pPropID).addClass("current-price-spec");
    //console.log("#PropItem_" + pPropID);

    //设置当前的价格与库存
    var _priceStockArr = $("#PropItem_" + pPropID).attr("data-price-stock").split("|");
    $("#GoodsPriceB").html(_priceStockArr[0]);
    $("#GoodsStockB").html(_priceStockArr[1]);
}
...
/**
 * 初始化商品照片上传 插件
 * @param {any} pJsonGoodsImgList 商品照片列表 Json对象
 */
function initGoodsImgUploadList(pJsonGoodsImgList) {
    ...
    for (var i = 1; i < pJsonGoodsImgList.length + 1; i++) {
        $("#GoodsImgPre_" + i).attr("src", "//" + pJsonGoodsImgList[i - 1].ImgPath);
        $("#HidUploadGoodsVal_" + i).val(pJsonGoodsImgList[i - 1].UploadGuid + "|" + pJsonGoodsImgList[i - 1].ImgPath);

        var mImgPathDomain = pJsonGoodsImgList[i - 1].ImgPath;
...
/**
 * 初始化店铺商品类别
 * @param pJsonShopGoodsTypeList 店铺商品类别列表 Json对象
 * */
function initShopGoodsType(pJsonShopGoodsTypeList) {

    if (pJsonShopGoodsTypeList == 0) {
        return;
    }
    console.log(pJsonShopGoodsTypeList);

    //父级
    var _goodsTypeFather = pJsonShopGoodsTypeList.GoodsTypeFather;
    $("#ShopGoodsTypeTop").val(_goodsTypeFather.ShopGoodsTypeID);

    //根据父级的值为子级列表赋值
    chgSecondShopGoodsType();

    //子级
    var _goodsTypeSub = null;
    //try {
    _goodsTypeSub = pJsonShopGoodsTypeList.GoodsTypeSub;

    if (_goodsTypeSub != undefined) {
        $("#ShopGoodsTypeSecond").val(_goodsTypeSub.ShopGoodsTypeID);
    }
...
/**
 * 初始化商品类别信息
 * @param {any} pJsonGoodsTypeList 商品类目列表 Json对象
 */
function initGoodsTypeMsg(pJsonGoodsTypeList) {

    var _goodsTypeFirst = pJsonGoodsTypeList.GoodsTypeFirst;
    var _goodsTypeSecond = pJsonGoodsTypeList.GoodsTypeSecond;
    var _goodsTypeThird = pJsonGoodsTypeList.GoodsTypeThird;

    $("#GoodsTypeSpan_First").html(_goodsTypeFirst.GoodsTypeName);
    $("#hidFirstGoodsTypeID").val(_goodsTypeFirst.GoodsTypeID);
    $("#GoodsTypeSpan_Second").html(_goodsTypeSecond.GoodsTypeName);
    $("#hidSecondGoodsTypeID").val(_goodsTypeSecond.GoodsTypeID);
    $("#GoodsTypeSpan_Third").html(_goodsTypeThird.GoodsTypeName);
    $("#hidThirdGoodsTypeID").val(_goodsTypeThird.GoodsTypeID);
}

/**
 * 初始化商品类目必填属性
 * @param pGoodsTypeNeedPropVal 商品必填属性 ["袖长_无袖^腰型_高腰^廓形_A型^当季热销_嘻嘻哈哈^裙长_超短裙"]
 * */
function initGoodsTypeNeedPropVal(pGoodsTypeNeedPropVal) {

    //商品必填属性值数组
    var _goodsTypeNeedPropValArr = new Array();
    //分解分割字符串数组
    var _goodsTypeNeedPropArr = new Array();
    if (pGoodsTypeNeedPropVal.indexOf("^") >= 0) {
        _goodsTypeNeedPropArr = pGoodsTypeNeedPropVal.split('^');
        //循环构造值
        for (var i = 0; i < _goodsTypeNeedPropArr.length; i++) {
            _goodsTypeNeedPropValArr[i] = _goodsTypeNeedPropArr[i].split('_')[1];
        }
    }
    else {
        _goodsTypeNeedPropValArr[0] = pGoodsTypeNeedPropVal.split('_')[1];
    }
    //console.log(_goodsTypeNeedPropValArr);

    //为下拉框和文本框赋值
    var _nameGoodsPropArr = document.getElementsByName("NameGoodsProp");

    for (var j = 0; j < _nameGoodsPropArr.length; j++) {
        //赋值给文本框
        $(_nameGoodsPropArr[j]).val(_goodsTypeNeedPropValArr[j]);
    }
}

/**
 * 初始化加载 类目自定义属性
 * @param {any} pGoodsTypeCustomPropArr 自定义数组拼接字符串 [ 呵呵时有时无|有点^呵呵时有时无|嘻嘻^在外面|是我们 ]
 */
function initGoodsTypeCustomProp(pGoodsTypeCustomPropArr) {

    if (pGoodsTypeCustomPropArr == undefined) {
        return;
    }

    //加载类目自定义属性
    mGoodsMypropArr = pGoodsTypeCustomPropArr;

    var _xhtml = "";
    if (pGoodsTypeCustomPropArr.indexOf("^") >= 0) {
        var _goodsTypeCustomPropArr = pGoodsTypeCustomPropArr.split('^');
        //循环构造显示代码
        for (var i = 0; i < _goodsTypeCustomPropArr.length; i++) {
            var _goodsTypeCustomPropValArr = _goodsTypeCustomPropArr[i].split('|');
            _xhtml += "<li>";
            _xhtml += "<span>" + _goodsTypeCustomPropValArr[0] + ":</span>";
            _xhtml += "" + _goodsTypeCustomPropValArr[1] + "";
            _xhtml += "</li>";
        }
    }
    else {
        var _goodsTypeCustomPropValArr = pGoodsTypeCustomPropArr.split('|');
        _xhtml += "<li>";
        _xhtml += "<span>" + _goodsTypeCustomPropValArr[0] + ":</span>";
        _xhtml += "" + _goodsTypeCustomPropValArr[1] + "";
        _xhtml += "</li>";
...
function saveGooSpecParamPre() {
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "2", "GoodsID": $("#hidGoodsID").val().trim(), "GoodsSpecPriceStockArr": mGoodsSpecPriceStockArr, "SpecTitle": mGoodsSpecTitle, "SpecAttrName": mGoodsPropTitle,
    };
    console.log(dataPOST);
...
function saveGooSpecParam() {
    ...
    console.log("_gooSpecParamJson=" + _gooSpecParamJson);
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "4", "GoodsID": $("#hidGoodsID").val().trim(), "GooSpecParamJson": escape(_gooSpecParamJson), "SpecTitle": mGoodsSpecTitle, "SpecAttrName": mGoodsPropTitle,
    };
    console.log(dataPOST);
...
function nextStep() {
    ...
    console.log("hidThirdGoodsTypeID=" + hidThirdGoodsTypeID);
 ...
 * 加载类目必填属性
 * */
function initGoodsTypeNeedProp(pCallBack) {
            ...
            if (reTxt != "") {
                ...
                for (var i = 0; i < _needPropArr.length; i++) {
                    ...
                    if (_needPropArr[i].InputType == "select") {
                        ...
                        _html += "<select name=\"NameGoodsProp\" data-inputtype=\"select\" data-goodsprop=\"" + _needPropName + "\" class=\"goods-prop-select\">";
                        ...
                        _html += "<input name=\"NameGoodsProp\" data-inputtype=\"text\" data-goodsprop=\"" + _needPropName + "\" type=\"text\" class=\"goods-prop-txt\" />";
                    }
                    _html += "</li>";
                    //console.log(_html);
                }
...
function delAllSpecPriceStock() {
    ...
    confirmWinWidth("确定要删除所有规格信息吗？", function () {
        ...
        //构造POST参数
        var dataPOST = {
            "Type": "3", "GoodsID": $("#hidGoodsID").val().trim(),
        };
        console.log(dataPOST);
 ...
 */
function delGoodsImgMsg(pUploadGuid) {

    //构造POST参数
    var dataPOST = {
        "Type": "1", "UploadGuid": pUploadGuid
    };
    console.log(dataPOST);
    ...
    $.ajax({
        type: "POST",
        url: "../Goods/GooGoodsImg?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log(reTxt);
...
function xhtmlSelGiftList() {
    ...
    console.log(mJsonGooGiftMsgList);
    console.log(mSelGiftIDArr);
 ...
 * */
function loadShopGoodsType(pCallBack) {
    ...
    $.ajax({
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                //初始化运费模板值
                $("#FtIDSelect").val(mFtID);

                console.log("初始化运费模板mFtID=" + mFtID);
 ...
 * 保存发布商品信息
 * */
function saveGoodsMsg() {

    //获取表单值
    var hidThirdGoodsTypeID = $("#hidThirdGoodsTypeID").val().trim();
    var hidGoodsMsgGuid = $("#hidGoodsMsgGuid").val().trim();
    var hidGoodsID = $("#hidGoodsID").val().trim(); //商品ID
    ...
    for (var i = 0; i < _nameGoodsPropArr.length; i++) {
        ...
        //console.log($(_nameGoodsPropArr[i]).val());
        var _goodsPropValue = "";
    ...
    //------分销------//
    var IsDistri = "";//$("#IsDistri").is(':checked');
    var DistriMoney = "";//$("#DistriMoney").val().trim();
    //console.log("提交IsDistri=" + IsDistri + "|DistriMoney=" + DistriMoney);
    ////alert("IsDistri=" + IsDistri + "|DistriMoney=" + DistriMoney);
    //if (IsDistri) {
    //    if (DistriMoney == "") {
    //        toastWin("【分销佣金】不能为空！");
    //        $("#DistriMoney").focus();
    //        return;
    //    }
    //    if (parseFloat(DistriMoney) <= 0) {
    //        toastWin("【分销佣金】不能小于等于0！");
    //        return;
    //    }
    //}
