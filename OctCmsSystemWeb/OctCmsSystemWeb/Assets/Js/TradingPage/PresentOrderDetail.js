...
function setValToXhtml(pReTxtJson) {
    ...
    var _IsRefund = "true-已退款"
    if (PresentOrderMsg.IsRefund == "false") {
        _IsRefund = "false-未退款"
    }
    $("#IsRefund").html(_IsRefund);

    $("#BuyerUserID").html(PresentOrderMsg.BuyerUserID);
    ...
    if (ShopMsg != undefined) {
        $("#ShopID").html(ShopMsg.ShopID);
        $("#ShopName").html("<a href=\"../UserGoodsShopPage/ShopMsgDetail?UserID=" + PresentOrderMsg.ShopUserID + "\" target=\"_blank\">" + ShopMsg.ShopName + "(" + PresentOrderMsg.ShopUserID + ")</a>");
        $("#ShopMobile").html(ShopMsg.ShopMobile);
    }
