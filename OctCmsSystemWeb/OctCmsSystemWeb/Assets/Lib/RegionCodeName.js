 ...
 * 初始化省市县区域范围值
 * */
function initRegionCodeName(pID_Region_province = "region_province", pID_region_city = "region_city") {
    $.getJSON("../../Assets/Data/RegionNameCode.json", function (dataBack) {
        data = dataBack;
        console.log("区域Json文件加载成功");
        ...
        //初始化下拉列表
        initSelect2ProvinceValue(pID_Region_province, pID_region_city);
 ...
 */
function initRegionCodeNameDefaultVal(pProvinceRegionCode, pCityRegionCode, pCountryRegionCode, pID_Region_province = "region_province", pID_region_city = "region_city", pID_region_county = "region_county") {
    $.getJSON("../../Assets/Data/RegionNameCode.json", function (dataBack) {
        data = dataBack;
        console.log("区域Json文件加载成功");
        ...
        //初始化当前的 省份，城市，区县 值
        setInitAreaValue(pProvinceRegionCode, pCityRegionCode, pCountryRegionCode, pID_Region_province, pID_region_city, pID_region_county);
 ...
 * 初始化下拉表值 没有初始化值
 */
function initSelect2ProvinceValue(pID_Region_province = "region_province", pID_region_city = "region_city") {

    var region_province = document.getElementById(pID_Region_province);
    ...
    $("#" + pID_Region_province + "").change(function () {
        //加载城市列表 
        onProvinceChange("",pID_Region_province);
    });
    $("#" + pID_region_city + "").change(function () {
        //加载区县列表
        onCityChange("", pID_region_city);
    });
}
 ...
 */
function setInitAreaValue(pProviceCodeValue, pCityCodeValue, pCountyValue, pID_Region_province = "region_province", pID_region_city = "region_city", pID_region_county = "region_county") {

    //初始化下拉表值，加载省份
    initSelect2ProvinceValue(pID_Region_province, pID_region_city, pID_region_county, pID_Region_province, pID_region_city, pID_region_county);

    //省份
    $("#" + pID_Region_province + "").find("option[value='" + pProviceCodeValue + "']").attr("selected", true);
    //加载城市列表 
    onProvinceChange("",pID_Region_province);

    //城市
    $("#" + pID_region_city + "").find("option[value='" + pCityCodeValue + "']").attr("selected", true);
    //加载区县列表
    onCityChange("", pID_region_city);

    //区县
    $("#" + pID_region_county + "").find("option[value='" + pCountyValue + "']").attr("selected", true);
}
 ...
 */
function onProvinceChange(obj,pID_Region_province = "region_province") {

    if (obj == undefined || obj == "") {
        obj = document.getElementById("" + pID_Region_province +"");
    }
 ...
 */
function onCityChange(obj,pID_region_city = "region_city") {

    if (obj == undefined || obj == "") {
        obj = document.getElementById("" + pID_region_city + "");
    }
