...
//AjaxURL
var mAjaxUrl = "../Trading/SettleApply";


/**------初始化------**/
$(function () {

    if ($("#hidSettleStatus").val().trim() != "") {
        var _selOption = $("#SettleStatus_se").find("option");
        for (var i = 0; i < _selOption.length; i++) {
            if ($("#hidSettleStatus").val().trim() == $(_selOption[i]).val().trim()) {

                $(_selOption[i]).attr("selected", true);
                $("#SettleStatus_se").trigger('changed.selected.amui');
                break;
            }
        }
    }
...
var searchContent = function () {
    ...
    var RealTransSum_se = $("#RealTransSum_se").val().trim();
    var ApplyName_se = $("#ApplyName_se").val().trim();
    var ApplyTel_se = $("#ApplyTel_se").val().trim();
    var SettleStatus_se = $("#SettleStatus_se").val().trim();
    var DisposeMan_se = $("#DisposeMan_se").val().trim();
    var SettleMemo_se = $("#SettleMemo_se").val().trim();
    var DisposeDate_se = $("#DisposeDate_se").val().trim();
    var ApplyDate_se = $("#ApplyDate_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "SettleID": SettleID_se, "ShopUserID": ShopUserID_se, "RealTransSum": RealTransSum_se,
        "ApplyName": ApplyName_se, "ApplyTel": ApplyTel_se, "SettleStatus": SettleStatus_se, "DisposeMan": DisposeMan_se, "SettleMemo": SettleMemo_se, "DisposeDate": DisposeDate_se, "ApplyDate": ApplyDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        var _SettleStatus = indexDataPage.SettleStatus;
        if (indexDataPage.SettleStatus == "处理中") {
            _SettleStatus = "<font color=\"red\">" + indexDataPage.SettleStatus + "</font>"
        }

        myJsVal += "<tr>";
        myJsVal += " <td><a href=\"../TradingPage/SettleDetail?SAID=" + indexDataPage.SettleID + "\" target=\"_blank\">" + indexDataPage.SettleID + "</a></td>";
        ...
        myJsVal += " <td>" + indexDataPage.RealTransSum + "</td>";
        myJsVal += " <td>" + indexDataPage.ApplyName + "</td>";
        myJsVal += " <td>" + indexDataPage.ApplyTel + "</td>";
        myJsVal += " <td>";
        myJsVal += "     " + _SettleStatus + "";
        myJsVal += "     <i style=\"color: #939393;display: block\">" + indexDataPage.SettleMemo + "</i>";
        myJsVal += " </td>";
        myJsVal += " <td>";
        myJsVal += "     " + indexDataPage.DisposeDate + "";
        myJsVal += "     <i style=\"color: #939393;display: block\">" + indexDataPage.ApplyDate + "</i>";
        myJsVal += " </td>";
        myJsVal += " <td>";
        if (indexDataPage.SettleStatus == "处理中") {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-danger am-round\" onclick=\"window.open('../TradingPage/SettleDispose?SAID=" + indexDataPage.SettleID + "')\">处理</button>";
        }
        else {
            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\"  onclick=\"window.open('../TradingPage/SettleDetail?SAID=" + indexDataPage.SettleID + "')\">详细</button>";
        }
        myJsVal += " </td>";
        myJsVal += "</tr>";
