...
//AjaxURL
var mAjaxUrl = "../MallAjax/PresentExchange";
...
function chgTabPrice() {
    ...
    if (mPresentPriceSort == "PresentPriceAsc") {
        ...
        //切换选项卡
        clickItemSearchWhere2('PresentPriceDesc')

    }
    else {
        mPresentPriceSort = "PresentPriceAsc";
        //切换选项卡
        clickItemSearchWhere2('PresentPriceAsc')
    }
...
/**
 * 选择排序方式
 * @param pPageOrderName    //排序方式 综合 Syn ，价格 PresentPriceAsc,PresentPriceDesc 库存降序 StockNum
 */
function clickItemSearchWhere2(pPageOrderName) {
    ...
    //排序方式 综合 Syn ，价格 PriceAsc,PriceDesc 库存降序 StockNum
...
function pageSuccess(curPageDataJson) {
    ...
    for (var i = 0; i < dataPageArr.length; i++) {
        ...
        if (dataPage.PresentTitle.length > 25) {
            dataPage.PresentTitle = dataPage.PresentTitle.substring(0, 25) + "...";
        }
        ...
        myJsVal += "     <div>剩余:" + dataPage.StockNum + "</div>";
        myJsVal += " </div>";
        myJsVal += "</li>";
    }
