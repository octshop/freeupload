...
//AjaxURL
var mAjaxUrl = "../LuckyDraw/LuckyDrawJoinMsg";
...
var searchContent = function () {
    ...
    //翻页所用
    var strPOSTSePush = {
        "LuckydrawID": LuckydrawID_se, "UserNick": UserNick_se, "LinkMobile": LinkMobile_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    var myJsVal = "";    //循环构造    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";        myJsVal += "  <td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.LuckyDrawJoinID + "\" />";        myJsVal += " <td><a href=\"../LuckyDrawPage/LuckyDrawMsgDetail?LID=" + indexDataPage.LuckydrawID + "\" target=\"_blank\">" + indexDataPage.LuckydrawID + "</a></td>";        myJsVal += " <td>";        myJsVal += "     <img src=\"" + indexDataPage.HeaderImg + "\" />";        myJsVal += "     " + indexDataPage.UserNick + "";        myJsVal += " </td>";        myJsVal += " <td>" + indexDataPage.LinkMobile + "</td>";        myJsVal += " <td>" + indexDataPage.WriteDate + " <font color=\"red\">" + _WriteDateDay + "</font></td>";        myJsVal += " <td>";        myJsVal += "     <button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"delLuckyDrawJoinMsgArr('" + indexDataPage.LuckyDrawJoinID + "')\">删除</button>";        myJsVal += " </td>";        myJsVal += "</tr>";    }    //alert(myJsVal);    //-----分页控制条显示代码-------//    var pageBarXhtml = "";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"PrePage()\">«</a></li>";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('1')\">1</a></li>";    pageBarXhtml += "  <li><span>...</span></li>";    if ((intPageCurrent - 2) > 0) {
...
/**
 * 批量删除抽奖参与信息
 * @param {any} pLuckyDrawJoinIDArr  抽奖参与信息ID拼接，用“^”连接
 */
function delLuckyDrawJoinMsgArr(pLuckyDrawJoinIDArr) {

    confirmWinWidth("确认要删除吗？", function () {

        //构造POST参数
        var dataPOST = {
            "Type": "2", "LuckyDrawJoinIDArr": pLuckyDrawJoinIDArr,
        };
 ...
 * 删除选择的抽奖参与信息
 * */
function delAllLuckyDrawJoinMsgArr() {
    ...
    delLuckyDrawJoinMsgArr(_SelectValArr);
