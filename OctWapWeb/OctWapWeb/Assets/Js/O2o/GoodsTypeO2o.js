...
$(function () {
    ...
    $("#GoodsTypeTtitleB").html($("#hidGoodsTypeNameSec").val().trim());

    // 加载第三级商品类目,根据第二级类目ID
    loadGoodsTypeThirdBySec(function (pBackData) {
 ...
 * 加载第三级商品类目,根据第二级类目ID
 * */
function loadGoodsTypeThirdBySec(pCallBack) {
    //构造POST参数
    var dataPOST = {
        "Type": "2", "GoodsTypeIDSec": mGoodsTypeIDSec, "IsEntity": "true",
    };
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                for (var i = 0; i < GooGoodsTypeThirdLevelList.length; i++) {
                    myJsVal += "<span class=\"goods-type-item\" onclick=\"clickItemSearchWhere1('" + GooGoodsTypeThirdLevelList[i].GoodsTypeID + "')\">" + GooGoodsTypeThirdLevelList[i].GoodsTypeName + "</span>";
                }
                $("#AllWhereType1").html(myJsVal);
