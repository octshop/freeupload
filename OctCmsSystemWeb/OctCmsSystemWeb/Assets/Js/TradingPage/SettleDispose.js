...
$(function () {
    ...
    //初始化上传结算转账凭证照片
    initUploadSettleTransferVoucherImg();
        ...
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                if (_jsonReTxt.SettleStatus == "处理中") {
                    ...
                    window.location.href = "../TradingPage/SettleDetail?SAID=" + mSettleID;
                    return;
                }
                ...
                //初始化商家结算资料
                initSettleShopMsg(_jsonReTxt.ShopUserID);
 ...
 * 提交结算处理完成
 * */
function submitSettleFinishPlatform() {
    ...
    //获取表单
    var DisposeMan = $("#DisposeMan").val().trim();
    var RealTransSum = $("#RealTransSum").val().trim();
    var SettleMemo = $("#SettleMemo").val().trim();
    var TransferVoucherImg = mImgPathDomain;
    //判断输入
    if (DisposeMan == "" || RealTransSum == "") {
        toastWin("【处理者姓名】【实际转账金额】不能为空");
        return;
    }
    if (parseFloat(RealTransSum) <= 0) {
        toastWin("【实际转账金额】不能小于等于零！");
        return;
    }
    if (TransferVoucherImg == "") {
        toastWin("请上传【转账凭证照片】！");
        return;
    }
    ...
    //构造POST参数
    var dataPOST = {
        "Type": "4", "SettleID": mSettleID, "DisposeMan": DisposeMan, "RealTransSum": RealTransSum, "TransferVoucherImg": TransferVoucherImg, "SettleMemo": SettleMemo,
    };
    console.log(dataPOST);
    //加载提示
    $("#BtnSubmitSettle").attr("disabled", true);
    $("#BtnSubmitSettle").val("…提交中…");
    ...
    $.ajax({
        ...
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("提交结算处理完成=" + reTxt);

            //移除加载提示
            $("#BtnSubmitSettle").attr("disabled", false);
            $("#BtnSubmitSettle").val("提交处理结果");
 ...
 */
function initSettleShopMsg(pShopUserID) {


    //构造POST参数
    var dataPOST = {
        "Type": "2", "ShopUserID": pShopUserID, "IsCheck": "pass",
    };
    console.log(dataPOST);
    ...
    $.ajax({
        type: "POST",
        url: "../Trading/SettleShopMsg?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            ...
            if (reTxt != "") {
                ...
                if (_jsonReTxt.BankAccount == "" || _jsonReTxt.BankAccount == null) {
                    $("#BankAccount").html("商家结算账号信息异常，请检查！");
                }
                else {
                    //显示赋值
                    $("#BankAccount").html(_jsonReTxt.BankAccount);
                    $("#BankAccName").html(_jsonReTxt.BankAccName);
                    $("#OpeningBank").html(_jsonReTxt.OpeningBank);
                    $("#LinkMan").html(_jsonReTxt.LinkMan);
                    $("#MobileNumber").html(_jsonReTxt.MobileNumber);
                    $("#CompanyName").html(_jsonReTxt.CompanyName);
                    $("#CompanyAddr").html(_jsonReTxt.RegionNameArr + "_" + _jsonReTxt.CompanyAddr);
                }
            }
            else {
 ...
 * 初始化上传结算转账凭证照片
 * */
function initUploadSettleTransferVoucherImg() {

    console.log("mSettleID=" + mSettleID);
    if (mSettleID == 0 || mSettleID == "") {
        return;
    }

    //构造POST参数
    var _dataPost = "Type=1&ExtraData=" + mSettleID;


    $('#fileupload_0').fileupload({
        url: "../FileUpload/SettleTransferVoucherImg?" + _dataPost + "&rnd=" + Math.random(),
        //dataType: 'json',
        dataType: "text",
        //------------处理上传成功后的事件------------//
            ...
            $("#TransferVoucherImg").show();
            $("#TransferVoucherImg").html("<a href=\"//" + mImgPathDomain + "\" target=\"_blank\"><img src=\"//" + mImgPathDomain + "\" /></a>");
