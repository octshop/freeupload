...
//AjaxURL
var mAjaxUrl = "../Trading/BuyerIncomeExpense";
...
var searchContent = function () {
    ...
    var InExMsgID_se = $("#InExMsgID_se").val().trim();
    var BuyerUserID_se = $("#BuyerUserID_se").val().trim();
    var IncomeSum_se = $("#IncomeSum_se").val().trim();
    var ExpenseSum_se = $("#ExpenseSum_se").val().trim();
    var CurrentBalance_se = $("#CurrentBalance_se").val().trim();
    var InExType_se = $("#InExType_se").val().trim();
    var InExMemo_se = $("#InExMemo_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "InExMsgID": InExMsgID_se, "BuyerUserID": BuyerUserID_se, "IncomeSum": IncomeSum_se,
        "ExpenseSum": ExpenseSum_se, "CurrentBalance": CurrentBalance_se, "InExType": InExType_se, "InExMemo": InExMemo_se, "ExtraData": ExtraData_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        var _ExtraDataHref = "#";
        if (indexDataPage.InExType == "Order" || indexDataPage.InExType == "Refund") {
            _ExtraDataHref = "../TradingPage/OrderDetail?OID=" + indexDataPage.ExtraData;
        }
        else if (indexDataPage.InExType == "WithDraw") {
            _ExtraDataHref = "../TradingPage/BuyerWithDrawDispose?WDID=" + indexDataPage.ExtraData;
        }
        else if (indexDataPage.InExType == "TransDividend") {
            _ExtraDataHref = "../UserGoodsShopPage/UserAccountDetail?UID=" + indexDataPage.BuyerUserID;
        }


        myJsVal += "<tr>";
        myJsVal += " <td>" + indexDataPage.InExMsgID + "</td>";
        myJsVal += " <td>" + indexDataPage.BuyerUserID + "<i style=\"color: #939393;display: block\"><a href=\"../UserGoodsShopPage/UserAccountDetail?UID=" + indexDataPage.BuyerUserID + "\" target=\"_blank\">" + indexDataPageExtra.BuyerNickName + "</a></i></td>";
        myJsVal += " <td>+ " + indexDataPage.IncomeSum + "</td>";
        myJsVal += " <td>- " + indexDataPage.ExpenseSum + "</td>";
        myJsVal += " <td>" + indexDataPage.CurrentBalance + "</td>";
        myJsVal += " <td>" + indexDataPageExtra.InExTypeName + "<br />(" + indexDataPage.InExType + ")</td>";
        myJsVal += " <td><a href=\"" + _ExtraDataHref + "\" target=\"_blank\">" + indexDataPageExtra.ExtraDataName + ":" + indexDataPage.ExtraData + "</a></td>";
