...
//AjaxURL
var mAjaxUrl = "../Present/PresentMsg";
...
var searchContent = function () {
    ...
    //翻页所用
    var strPOSTSePush = {
        "PresentID": PresentID_se, "PresentTitle": PresentTitle_se, "PresentStatus": PresentStatus_se,
        "StockNum": StockNum_se, "IsUnSale": IsUnSale_se, "IsPinkage": IsPinkage_se, "IsShopExpense": IsShopExpense_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    var myJsVal = "";    //循环构造    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        var _presentStatus = "出售中";
        if (indexDataPage.PresentStatus == "审") {
            _presentStatus = "<font color=\"red\">审核中</font>";
        }
        ...
        var _isPinkage = "否";
        if (indexDataPage.IsPinkage == "true") {
            _isPinkage = "是";
        }

        var _isShopExpense = "快递物流";
        if (indexDataPage.IsShopExpense == "true") {
            _isPinkage = "到店消费/自取";
        }
        ...
        myJsVal += "<tr>";        myJsVal += "<td><input type=\"checkbox\" name=\"SelCbItem\" id=\"SelCbItem_" + indexDataPage.PresentID + "\" /></td>";        myJsVal += " <td>" + indexDataPage.PresentID + "</td>";        myJsVal += " <td><a href=\"" + mOctWapWeb_AddrDomain + "/Present/PresentDetailPreMobileIframe?PID=" + indexDataPage.PresentID + "\" target=\"_blank\">" + indexDataPage.PresentTitle + "</a></td>";        myJsVal += " <td>&#165;" + indexDataPage.PresentPrice + "<br />" + _presentStatus + "</td>";        myJsVal += " <td>" + indexDataPage.StockNum + "</td>";        myJsVal += " <td>" + _isUnSale + "</td>";        myJsVal += " <td>" + _isPinkage + "</td>";        myJsVal += " <td>" + _isShopExpense + "</td>";        myJsVal += " <td>" + indexDataPage.WriteDate + "</td>";        myJsVal += " <td>";        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-secondary am-round\" onclick=\"window.location.href='../PresentPage/PresentEdit?PID=" + indexDataPage.PresentID + "'\"><span class=\"am-icon-pencil-square-o\"></span></button>";        if (indexDataPage.IsUnSale == "true") {            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning  am-round\" onclick=\"tglUnSalePresentMsg('" + indexDataPage.PresentID + "', 'false')\"><span class=\"am-icon-arrow-circle-down\"></span></button>";
        }        else {            myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-warning  am-round\" onclick=\"tglUnSalePresentMsg('" + indexDataPage.PresentID + "', 'true')\"><span class=\"am-icon-arrow-circle-up\"></span></button>";
        }        myJsVal += "<button class=\"table-btn am-btn am-btn-default am-btn-xs am-text-success am-round\" onclick=\"createQRCodeImg('" + indexDataPage.PresentID + "', '" + indexDataPage.PresentTitle + "')\">二维码</button>";        myJsVal += " </td>";        myJsVal += "</tr>";
...
/**
 * 上架或下架 礼品信息
 * @param {any} pPresentID 礼品ID
 * @param {any} pIsUnSale 是否下架 ( false  上架  / true  下架)
 */
function tglUnSalePresentMsg(pPresentID, pIsUnSale) {
    ...
    confirmWinWidth("确定要【" + _hintTxt + "】吗？", function () {
        ...
        //构造POST参数
        var dataPOST = {
            "Type": "2", "PresentID": pPresentID, "IsUnSale": pIsUnSale
        };
 ...
 * 生成二维码图片
 */
function createQRCodeImg(pPresentID, pPresentTitle) {

    var QRCodeTitle = "礼品领取分享二维码";
    var QRCodeTitleSub = pPresentTitle;
    var QRCodeDesc = "请将此二维码分享给好友或其他人，扫码识别，邀请好友领取礼品！";
    var QRCodeURL = mOctWapWeb_AddrDomain + "/Present/PresentDetail?PID=" + pPresentID + "";
