...
//AjaxURL
var mAjaxUrl = "../CommonCode/SystemMsg";
...
var searchContent = function () {
    ...
    var SysMsgID_se = $("#SysMsgID_se").val().trim();
    var OrderIDArr_se = $("#OrderIDArr_se").val().trim();
    var UserIDArr_se = $("#UserIDArr_se").val().trim();
    var MsgTitle_se = $("#MsgTitle_se").val().trim();
    var MsgContent_se = $("#MsgContent_se").val().trim();
    var MsgType_se = $("#MsgType_se").val().trim();
    var SendToType_se = $("#SendToType_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "SysMsgID": SysMsgID_se, "OrderIDArr": OrderIDArr_se, "UserIDArr": UserIDArr_se,
        "MsgTitle": MsgTitle_se, "MsgContent": MsgContent_se, "MsgType": MsgType_se, "SendToType": SendToType_se, "ExtraData": ExtraData_se, "IsRead": IsRead_se, "WriteDate": WriteDate_se
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        myJsVal += "<tr>";
        myJsVal += " <td>" + indexDataPage.SysMsgID + "</td>";
        myJsVal += " <td>" + indexDataPage.MsgType + "<br />(" + indexDataPageExtra.MsgTypeName + ")</td>";
        myJsVal += " <td><a href=\"#\" target=\"_blank\">" + indexDataPage.OrderIDArr + "</a></td>";
        myJsVal += " <td><a href=\"#\" target=\"_blank\">" + indexDataPage.UserIDArr + "</a></td>";
        myJsVal += " <td>" + indexDataPage.SendToType + "(" + indexDataPageExtra.SendToTypeName + ")</td>";
 ...
 */
function delSystemMsgMonthAgo(pMonthNumAgo) {
    ...
    confirmWinWidth("确定要删除吗？", function () {
        ...
        $.ajax({
            ...
            dataType: "html",
            success: function (reTxt, status, xhr) {
                console.log("删除指定几个月之前的 系统异常信息=" + reTxt);
                if (reTxt != "") {
