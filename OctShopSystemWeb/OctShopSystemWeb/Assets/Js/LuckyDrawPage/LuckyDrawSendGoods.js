...
//AjaxURL
var mAjaxUrl = "../LuckyDraw/LuckyDrawSendGoods";
...
var searchContent = function () {
    ...
    var ExpressName_se = $("#ExpressName_se").val().trim();
    var ExpressNumber_se = $("#ExpressNumber_se").val().trim();
    var SendTelNumber_se = $("#SendTelNumber_se").val().trim();
    var SendGoodsMemo_se = $("#SendGoodsMemo_se").val().trim();
    ...
    //翻页所用
    var strPOSTSePush = {
        "LuckyDrawResultID": LuckyDrawResultID_se, "ExpressName": ExpressName_se, "ExpressNumber": ExpressNumber_se, "SendTelNumber": SendTelNumber_se, "SendGoodsMemo": SendGoodsMemo_se, "WriteDate": WriteDate_se,
    };
    //将对象添加到分类对象中
...
function jsonToXhtml(pJsonObject) {
    ...
    var myJsVal = "";    //循环构造    for (var i = 0; i < json.DataPage.length; i++) {
        ...
        var _WriteDateDay = Math.abs(dateDiffDay(getTodayDate(), indexDataPage.WriteDate, false));
        if (Math.abs(_WriteDateDay) > 20 || _WriteDateDay <= 0) {
            _WriteDateDay = "";
        }


        myJsVal += "<tr>";        myJsVal += "  <td><a href=\"javascript:void(0)\" onclick=\"openWin('" + indexDataPage.LuckyDrawResultID + "')\">" + indexDataPage.LuckyDrawResultID + "</a></td>";        myJsVal += "  <td>";        myJsVal += "" + indexDataPage.ExpressName + "";        myJsVal += "  </td>";        myJsVal += "  <td>" + indexDataPage.ExpressNumber + "</td>";        myJsVal += "  <td>" + indexDataPage.SendTelNumber + "</td>";        myJsVal += "  <td>" + indexDataPage.SendGoodsMemo + "</td>";        myJsVal += "  <td>" + indexDataPage.WriteDate + " <font color=\"red\">" + _WriteDateDay + "</font></td>";        myJsVal += "</tr>";    }    //alert(myJsVal);    //-----分页控制条显示代码-------//    var pageBarXhtml = "";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"PrePage()\">«</a></li>";    pageBarXhtml += " <li><a href=\"javascript:void(0)\" onclick=\"NumberPage('1')\">1</a></li>";    pageBarXhtml += "  <li><span>...</span></li>";    if ((intPageCurrent - 2) > 0) {
 ...
 * 打开中奖详情窗口
 */
function openWin(pLuckyDrawResultID) {

    //打开Dialog弹出窗口
    openCustomDialogWin("中奖详情", mPopWinHtml, 600);

    //初始化抽奖结果信息
    initLuckyDrawResult(pLuckyDrawResultID)
}

/**
 * 初始化抽奖结果信息
 * @param {any} pLuckyDrawResultID
 */
function initLuckyDrawResult(pLuckyDrawResultID) {

    //构造POST参数
    var dataPOST = {
        "Type": "2", "LuckyDrawResultID": pLuckyDrawResultID, "BuyerUserID": "0",
    };
    ...
    $.ajax({
        type: "POST",
        url:  "../LuckyDraw/LuckyDrawResult?rnd=" + Math.random(),
        data: dataPOST,
        dataType: "html",
        success: function (reTxt, status, xhr) {
            console.log("初始化抽奖结果信息=" + reTxt);

            if (reTxt != "") {
                var _jsonReTxt = JSON.parse(reTxt);

                var LuckyDrawResult = _jsonReTxt.LuckyDrawResult;
                var LuckyDrawJoinMsg = _jsonReTxt.LuckyDrawJoinMsg;

                $("#LuckyDrawResultID_Win").html(LuckyDrawResult.LuckyDrawResultID);
                $("#LuckyDrawID_Win").attr("href", "../LuckyDrawPage/LuckyDrawMsgDetail?LID=" + LuckyDrawResult.LuckydrawID);
                $("#LuckyDrawID_Win").html(LuckyDrawResult.LuckydrawID);
                $("#AwardsItem_Win").html(LuckyDrawResult.AwardsItem);
                $("#NickName_Win").html("<img src=\"" + LuckyDrawJoinMsg.HeaderImg + "\" />" + LuckyDrawJoinMsg.UserNick + "");
                $("#LinkMobile_Win").html(LuckyDrawJoinMsg.LinkMobile);
                $("#WriteDate_Win").html(LuckyDrawResult.WriteDate);
            }
        },
        error: function (xhr, errorTxt, status) {
            console.log("异步请求错误，errorTxt=" + errorTxt + " | status=" + status);
