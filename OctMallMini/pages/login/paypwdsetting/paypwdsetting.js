// pages/login//paypwdsetting/paypwdsetting.js
//==============引入相关的Js文件========//
...
Page({
   ...
   * 监听新密码的输入
   */
  bindInputPayPwd: function (e) {
    var _payPwdNew = e.detail.value;
    //console.log(_payPwdNew);
    this.setData({
      payPwdNew: _payPwdNew,
    });
  },
   ...
   * 重设支付密码
   */
  chgUserPayPwd: function () {

    if (this.data.payPwdNew == "" || this.data.smsVerifyCode == "") {
      mUtils.showToast("【新的支付密码】和【短信验证码】不能为空！");
      return;
    }
    if (this.data.payPwdNew.length < 6) {
      mUtils.showToast("登录支付必须6个字符以上！");
      return;
    }
